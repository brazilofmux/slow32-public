...
// Custom varargs f64 handler: allocates two consecutive registers individually,
// allowing f64 to split across the register/stack boundary when only one
// register remains. This prevents gaps in the callee's register save area.
static bool CC_SLOW32_VarArg_F64(unsigned ValNo, MVT ValVT, MVT LocVT,
                                  CCValAssign::LocInfo LocInfo,
                                  ISD::ArgFlagsTy ArgFlags, CCState &State) {
  static const MCPhysReg VarArgRegs[] = {
    SLOW32::R3, SLOW32::R4, SLOW32::R5, SLOW32::R6,
    SLOW32::R7, SLOW32::R8, SLOW32::R9, SLOW32::R10
  };

  // Allocate lo half from next available register
  MCRegister LoReg = State.AllocateReg(VarArgRegs);
  if (!LoReg)
    return false; // no regs left, fall through to CCAssignToStack

  // Allocate hi half from next available register
  MCRegister HiReg = State.AllocateReg(VarArgRegs);
  if (HiReg) {
    // Both halves in registers — standard assignment (caller uses getNextArgReg)
    State.addLoc(CCValAssign::getReg(ValNo, ValVT, LoReg, LocVT, LocInfo));
    return true;
  }

  // Split: lo in register, hi on stack (fills the last register, no gap)
  State.addLoc(
      CCValAssign::getCustomReg(ValNo, ValVT, LoReg, MVT::i32, LocInfo));
  int64_t HiOffset = State.AllocateStack(4, Align(4));
  State.addLoc(
      CCValAssign::getCustomMem(ValNo, ValVT, HiOffset, MVT::i32, LocInfo));
  return true;
}
...
SDValue SLOW32TargetLowering::LowerFormalArguments(
  ...
  if (isVarArg) {
    ...
    // Save all arg registers (R3-R10) not consumed by fixed params.
    // The caller's CC_SLOW32_VarArgCall packs args sequentially with no
    // gaps (splitting f64 at the register/stack boundary when needed),
    // so the save area + stack args form one contiguous stream for va_arg.
    unsigned NumRegsLeft = FirstVAReg < NumSLOW32ArgRegs ? NumSLOW32ArgRegs - FirstVAReg : 0;
    ...
    if (NumRegsLeft) {
      ...
      for (unsigned RegIdx = FirstVAReg; RegIdx < NumSLOW32ArgRegs; ++RegIdx)
...
SDValue SLOW32TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  for (unsigned I = 0, E = ArgLocs.size(); I != E;) {
    ...
    // Handle custom varargs f64 split (lo in reg, hi on stack)
    if (VA.needsCustom() && VA.getValVT() == MVT::f64) {
      ...
      assert(VA.isRegLoc() && HiVA.isMemLoc());
      ...
      // Lo half in register
      RegsToPass.emplace_back(VA.getLocReg(), Split.getValue(0));
      ...
      SDValue Offset = DAG.getConstant(HiVA.getLocMemOffset(), DL, PtrVT);
      SDValue Ptr = DAG.getNode(ISD::ADD, DL, PtrVT, StackPtr, Offset);
      SDValue Store = DAG.getStore(Chain, DL, Split.getValue(1), Ptr,
                                   MachinePointerInfo());
