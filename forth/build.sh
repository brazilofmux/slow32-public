...
# Run: the prelude, then interactive stdin.  The emulator serves
# S32_STDIN_PREFIX before the real stdin, so it is the only process on the
# terminal: `cat prelude.fth - | emu` left cat holding the tty after BYE
# (the shell waits for every member of a pipeline).
echo "Running..."
S32_STDIN_PREFIX=prelude.fth exec $EMU kernel.s32x
