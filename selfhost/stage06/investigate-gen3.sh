...
for f in "$STAGE5_CC" "$STAGE5_AS" "$STAGE5_LD"; do
    [[ -f "$f" ]] || { echo "Missing: $f" >&2; exit 1; }
done

WORKDIR="$(mktemp -d /tmp/stage06-gen3-investigate.XXXXXX)"
...
"$DBT" "$STAGE5_AS" "$CRT0_SRC" "$WORKDIR/crt0.s32o" >/dev/null 2>&1
"$DBT" "$STAGE5_AS" "$MMIO_NO_START_SRC" "$WORKDIR/mmio_no_start.s32o" >/dev/null 2>&1

LIBC_OBJS=""
for name in string_extra string_more ctype convert stdio malloc; do
    "$DBT" "$STAGE5_CC" "$LIBC_DIR/${name}.c" "$WORKDIR/${name}.s" >/dev/null 2>&1
    "$DBT" "$STAGE5_AS" "$WORKDIR/${name}.s" "$WORKDIR/${name}.s32o" >/dev/null 2>&1
    LIBC_OBJS="$LIBC_OBJS $WORKDIR/${name}.s32o"
done
"$DBT" "$STAGE5_CC" "$LIBC_DIR/start.c" "$WORKDIR/start.s" >/dev/null 2>&1
"$DBT" "$STAGE5_AS" "$WORKDIR/start.s" "$WORKDIR/start.s32o" >/dev/null 2>&1
...
# ============================================================
# Step 2: Build gen1 (stage05 compiles s12cc.c)
# ============================================================
echo "=== Step 2: Build gen1 (stage05 → s12cc.c) ==="

timeout 600 "$DBT" "$STAGE5_CC" "$SCRIPT_DIR/s12cc.c" "$WORKDIR/gen1.s" >/dev/null 2>&1
echo "  gen1.s: $(wc -l < "$WORKDIR/gen1.s") lines"

"$DBT" "$STAGE5_AS" "$WORKDIR/gen1.s" "$WORKDIR/gen1.s32o" >/dev/null 2>&1
"$DBT" "$STAGE5_LD" -o "$WORKDIR/gen1.s32x" --mmio 64K \
...
"$DBT" "$STAGE5_LD" -o "$WORKDIR/gen2.s32x" --mmio 64K \
