...
/* Helpers carry _d suffix to avoid clashing with the same-named
 * helpers in fpu_ops_a64.c — stage07's lower currently emits static
 * functions with global linkage. */
static double to_f64_d(unsigned int lo, unsigned int hi) {
    double d;
...
static void from_f64_d(double d, unsigned int *lo, unsigned int *hi) {
    uint64_t bits;
    memcpy(&bits, &d, 8);
...
static float to_f32_d(unsigned int bits) {
    float f; memcpy(&f, &bits, 4); return f;
}

static unsigned int from_f32_d(float f) {
...
void fpu_add_d(unsigned int a_lo, unsigned int a_hi,
    ...
    from_f64_d(to_f64_d(a_lo, a_hi) + to_f64_d(b_lo, b_hi), r_lo, r_hi);
}
...
void fpu_sqrt_d(unsigned int a_lo, unsigned int a_hi,
                unsigned int *r_lo, unsigned int *r_hi) {
    from_f64_d(sqrt(to_f64_d(a_lo, a_hi)), r_lo, r_hi);
}

int fpu_eq_d(unsigned int a_lo, unsigned int a_hi,
             unsigned int b_lo, unsigned int b_hi) {
    return to_f64_d(a_lo, a_hi) == to_f64_d(b_lo, b_hi) ? 1 : 0;
}
...
void fpu_cvt_d_s(unsigned int a, unsigned int *r_lo, unsigned int *r_hi) {
    from_f64_d((double)to_f32_d(a), r_lo, r_hi);
}

unsigned int fpu_cvt_s_d(unsigned int a_lo, unsigned int a_hi) {
    return from_f32_d((float)to_f64_d(a_lo, a_hi));
}
...
void fpu_cvt_l_d(unsigned int a_lo, unsigned int a_hi,
                 unsigned int *r_lo, unsigned int *r_hi) {
    int64_t v = (int64_t)to_f64_d(a_lo, a_hi);
    *r_lo = (unsigned int)v;
    *r_hi = (unsigned int)((uint64_t)v >> 32);
}
...
void fpu_cvt_lu_d(unsigned int a_lo, unsigned int a_hi,
                  unsigned int *r_lo, unsigned int *r_hi) {
    uint64_t v = (uint64_t)to_f64_d(a_lo, a_hi);
    *r_lo = (unsigned int)v;
    *r_hi = (unsigned int)(v >> 32);
}
