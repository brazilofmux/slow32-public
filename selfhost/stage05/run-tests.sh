...
if [[ "$RUN_FIXED_POINT" -eq 1 && -s "$S13CC_EXE" ]]; then
    ...
    if [[ ! -s "$WORKDIR/fp-gen2.s" ]]; then
        ...
        if grep -Eq '^hir_burg|^hir_burg_select|^hir_imm_sel' "$WORKDIR/fp-gen2-compile.log"; then
            echo "  gen2 self-compile stats:"
            grep -E '^hir_burg|^hir_imm_sel|^  ' "$WORKDIR/fp-gen2-compile.log" | head -n 24
        fi
        ...
        if [[ ! -s "$WORKDIR/fp-gen2.s32o" ]]; then
            ...
            if [[ ! -s "$WORKDIR/fp-gen2.s32x" ]]; then
                ...
                if [[ ! -s "$WORKDIR/fp-gen3.s" ]]; then
                    ...
                    if grep -Eq '^hir_burg|^hir_burg_select|^hir_imm_sel' "$WORKDIR/fp-gen3-compile.log"; then
                        echo "  gen3 self-compile stats:"
                        grep -E '^hir_burg|^hir_imm_sel|^  ' "$WORKDIR/fp-gen3-compile.log" | head -n 24
                    fi
