...
# Build stage05 tools: s32-as.s32x, s32-ar.s32x, s32-ld.s32x, slow32dump.s32x, slow32dis.s32x
# All compiled by stage04's cc.s32x, assembled by stage04 assembler,
# linked by stage04 linker. Uses stage05's own libc/runtime.
...
WORKDIR="$(mktemp -d /tmp/stage05-tools.XXXXXX)"
...
assemble() {
    ...
    set +e
    timeout "${EXEC_TIMMEOUT:-1200" "$EMU" "$STAGE4_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${EXEC_TIMMEOUT:-1200" "$EMU" "$STAGE4_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
