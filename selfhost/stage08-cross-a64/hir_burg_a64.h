/* hir_burg_a64.h -- BURG-style instruction selection for AArch64
 *
 ...
 *
 * Adapted from stage08-cross-x64/hir_burg_x64.h. The pattern set is
 * very similar — AArch64 supports register-immediate forms for
 * most arithmetic and logical ops too — but with these tweaks:
 *   - HI_MUL with immediate is dropped (no MUL-imm on AArch64;
 *     constants must be materialised into a register first)
 *   - The MEM nonterminal stands for [Xn + small unsigned imm] for
 *     LDR/STR, equivalent in spirit to RBP+disp32 on x86-64
 *   - SADDR materialisation uses ADRP+ADD (cost 2) rather than
 *     movabs imm64 — same number of instructions
 *
 * Nonterminals:
 *   BG_REG   = value in a register
 *   BG_IMM   = compile-time 32-bit constant (not yet materialised)
 *   BG_MEM   = frame-relative address [Xn + disp]   (Xn is FP/x29)
 *   BG_SADDR = symbol address (needs relocation)
 *   BG_STMT  = side-effect (no value produced)
 */

#ifndef HIR_BURG_A64_H
#define HIR_BURG_A64_H

#define BG_REG    0
#define BG_IMM    1
#define BG_MEM    2   /* frame-relative [Xn+disp]; replaces SLOW-32's BG_FADDR */
#define BG_SADDR  3
#define BG_STMT   4
#define BG_NNT    5
...
static int bg_foff[HIR_MAX_INST];  /* precomputed a64 frame offset (via cg_a64_offset) */
...
/* --- AArch64 pattern initialization --- */
...
static void bg_init(void) {
    ...
    bg_add_pat(BG_MEM,   HI_ALLOCA, -1, -1, 0);  /* [FP + offset] */
    bg_add_pat(BG_REG,   HI_ALLOCA, -1, -1, 1);  /* add r, FP, #offset */
    ...
    bg_add_pat(BG_REG,  HI_LOAD, BG_SADDR, -1, 3);  /* adrp+add+ldr */
    bg_add_pat(BG_REG,  HI_LOAD, BG_REG,   -1, 1);  /* ldr r, [Xn] */

    /* Addressing: STORE */
    bg_add_pat(BG_STMT, HI_STORE, BG_MEM,   BG_REG, 1);  /* str r, [Xn + #disp] */
    bg_add_pat(BG_STMT, HI_STORE, BG_SADDR, BG_REG, 3);  /* adrp+add+str */
    bg_add_pat(BG_STMT, HI_STORE, BG_REG,   BG_REG, 1);  /* str r, [Xn] */
    ...
    bg_add_pat(BG_REG,   HI_ADDI, BG_MEM,   -1, 1);  /* add r, FP, #disp */
    bg_add_pat(BG_REG,   HI_ADDI, BG_REG,   -1, 1);  /* add r, r, #imm12 */

    /* Binary with immediate operand.
     * AArch64 ADD/SUB take 12-bit unsigned immediates (optionally <<12);
     * AND/ORR/EOR take logical immediates.  Codegen falls back to
     * materialise+register form when the immediate doesn't fit.
     * Note: HI_MUL with imm is intentionally not registered — AArch64
     * has no MUL-imm form. */
    ...
    bg_add_pat(BG_REG, HI_BNOT, BG_REG, -1, 1);  /* a64 has NOT r (1 instr) */
    ...
    /* Widening (a64 only) */
    bg_add_pat(BG_REG,  HI_SEXT32, BG_REG, -1, 1);  /* sxtw */
    bg_add_pat(BG_REG,  HI_ZEXT32, BG_REG, -1, 1);  /* mov r32d, r32d */

    /* Varargs (a64 only) */
    bg_add_pat(BG_REG,  HI_VA_START, -1, -1, 3);
    bg_add_pat(BG_REG,  HI_VA_ARG,  BG_REG, -1, 3);
    bg_add_pat(BG_REG,  HI_VA_NEXT, BG_REG, -1, 3);

    /* Chain rules */
    bg_add_chain(BG_IMM,   BG_REG, 1);   /* mov r, imm */
    bg_add_chain(BG_MEM,   BG_REG, 1);   /* add r, FP, #disp */
    bg_add_chain(BG_SADDR, BG_REG, 2);   /* adrp r, sym ; add r, r, #lo12(sym) */
}
 ...
 *
 * bg_foff[] stores the a64-scaled frame offset for ALLOCA/ADDI chains.
 * Uses cg_a64_offset() for the SLOW-32→a64 offset scaling.
 * (cg_a64_offset is defined in codegen_a64.h, included after this file.) */
...
static void bg_compute_foff(void) {
    ...
    while (i < h_ninst) {
        k = h_kind[i];
        /* Frame offsets (a64-scaled) */
        if (k == HI_ALLOCA) {
            bg_foff[i] = cg_a64_offset(h_val[i]);
        } else if (k == HI_ADDI) {
            s1 = h_src1[i];
            sk = -1;
...
static void bg_label(void) {
    ...
    while (i < h_ninst) {
        ...
        while (pi < bg_ocount[k]) {
            ...
            /* a64 guard: MEM patterns require valid frame offset chain.
             * On a64, 32-bit displacement always fits, so no range check
