...
static void extract_symbols(int member_idx) {
    ...
    for (int i = 0; i < ehdr->e_shnum; i++) {
        ...
        for (int j = 0; j < nsym; j++) {
            ...
            if (nsyms >= MAX_SYMS) {
                fprintf(stderr, "ar-a64: too many symbols\n");
                exit(1);
            }
...
int main(int argc, char **argv) {
    if (argc < 4 || strcmp(argv[1], "rcs") != 0) {
        fprintf(stderr, "Usage: ar-a64 rcs output.a obj1.o obj2.o ...\n");
        return 1;
    }
    ...
    for (int i = 3; i < argc; i++) {
        if (nmembers >= MAX_MEMBERS) {
            fprintf(stderr, "ar-a64: too many members\n");
            return 1;
        }
        ...
        if (!m->data) {
            fprintf(stderr, "ar-a64: cannot read: %s\n", argv[i]);
            return 1;
        }
    ...
    if (!f) {
        fprintf(stderr, "ar-a64: cannot create: %s\n", outfile);
        return 1;
    }
    ...
    fclose(f);
    fprintf(stderr, "ar-a64: created %s (%d members, %d symbols)\n",
