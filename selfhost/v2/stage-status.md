...
| `stage06` | Subset-C archiver replacement (`.s32o -> .s32a`) | `selfhost/v2/stage05/run-pipeline.sh --mode stage6-ar-smoke` and `... stage6-ar-rc-smoke` and `... stage6-ar-tx-smoke` | in progress (smoke gate green on `c/rc/t/x`) | widen coverage to `d/m/v/p` parity and archive-scan parity (`cs`) |
| `stage07` | Subset-C linker replacement (`.s32o + .s32a -> .s32x`) | `selfhost/v2/stage07/run-spike.sh` | in progress (single-object/zero-reloc spike wired) | widen to relocations + archive member resolution |
