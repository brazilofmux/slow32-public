...
| `stage08` | Subset-C transition gate (archiver parity + `cc-min` spike) | `selfhost/v2/stage08/run-regression.sh` | in progress (archiver parity + first compiler-in-C spike green) | expand `cc-min` toward real parser/codegen (`cc.c`) while preserving gate |
