...
| `stage06` | Subset-C archiver replacement (`.s32o -> .s32a`) | `selfhost/v2/stage05/run-pipeline.sh --mode progressive-as-ar --test test1` | in progress (create smoke green; `rc` replace path implemented and covered by focused smoke mode) | widen coverage to `t/x/d/m/v/p` parity and unblock Stage5 self-assembly reliability for `s32-ar*.c` |
