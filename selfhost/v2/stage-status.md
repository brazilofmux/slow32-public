...
| `stage06` | Subset-C archiver replacement (`.s32o -> .s32a`) | `selfhost/v2/stage05/run-pipeline.sh --mode progressive-as-ar --test test1` | in progress (progressive smoke green with Stage6 archiver assembled via `asm.fth`; Stage5 self-assembly of Stage6 still faults) | isolate/fix Stage5 assembler corruption on Stage6 `s32-ar*.c` self-assembly path |
