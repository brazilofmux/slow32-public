...
524288 CONSTANT OUT-SZ      \ 512KB output buffer
...
0 CONSTANT ASM-SEC-NONE
1 CONSTANT ASM-SEC-TEXT
2 CONSTANT ASM-SEC-DATA
3 CONSTANT ASM-SEC-BSS
VARIABLE asm-sec

: ENSURE-TEXT ( -- )
    asm-sec @ ASM-SEC-TEXT <> IF
        S" .text" EMIT-LINE
        ASM-SEC-TEXT asm-sec !
    THEN ;

: ENSURE-DATA ( -- )
    asm-sec @ ASM-SEC-DATA <> IF
        S" .data" EMIT-LINE
        ASM-SEC-DATA asm-sec !
    THEN ;

: ENSURE-BSS ( -- )
    asm-sec @ ASM-SEC-BSS <> IF
        S" .bss" EMIT-LINE
        ASM-SEC-BSS asm-sec !
    THEN ;
...
\ For <file.h> — try selfhost/stage04/include/ then runtime/include/
: PP-RESOLVE-INCLUDE ( -- )
    ...
    ELSE 60 = IF   \ <file.h>
        1 tmp-d +!
        0 inc-fname-len !
        \ Build path: selfhost/stage04/include/<file>
        S" selfhost/stage04/include/" inc-fname SWAP CMOVE
        25 inc-fname-len !
        BEGIN
            tmp-d @ pp-line-len @ < IF
