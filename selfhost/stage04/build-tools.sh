...
compile() {
    ...
    set +e
    timeout "${EXEC_TIMEOUT:-4800}" "$EMU" "$STAGE4_CC" "$src" "$asm" >"$log" 2>&1
    local rc=$?
    set -e
...
assemble() {
    ...
    set +e
    timeout "${EXEC_TIMEOUT:-4800}" "$EMU" "$STAGE3_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${EXEC_TIMEOUT:-4800}" "$EMU" "$STAGE3_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
