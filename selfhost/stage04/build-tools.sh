...
# All compiled by stage04's cc.s32x, assembled by stage03 assembler,
...
compile() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-4800}" "$EMU" "$STAGE4_CC" "$src" "$asm" >"$log" 2>&1
    local rc=$?
    set -e
...
assemble() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-4800}" "$EMU" "$STAGE3_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${SELFHOST_TIMEOUT:-4800}" "$EMU" "$STAGE3_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
