...
choose_default_emu() {
    # Stage08 policy: only SELFHOST_EMU or the minimal trusted stage00/s32-emu.
    # No automatic fast interpreter or DBT.
...
run_exe() {
    ...
    local rc=0
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$exe" "$@" >"$log" 2>&1 || rc=$?
    if [[ "$rc" -eq 124 ]]; then
...
# ============================================================
# Step 1: Bootstrap — build runtime/libc using stage07 compiler
# ============================================================
echo "=== Step 1: Bootstrap ==="

STAGE7_CC="$SELFHOST_DIR/stage07/cc.s32x"
AS_EXE="$SELFHOST_DIR/stage07/s32-as.s32x"
LD_EXE="$SELFHOST_DIR/stage07/s32-ld.s32x"

[[ -f "$STAGE7_CC" ]] || { echo "Missing compiler (stage07): $STAGE7_CC" >&2; exit 1; }
...
if [[ -f "$BUILTINS_FP64_SRC" ]]; then
    run_exe "$AS_EXE" "$WORKDIR/builtins_fp64.log" "$BUILTINS_FP64_SRC" "$WORKDIR/builtins_fp64.s32o"
...
run_exe "$STAGE7_CC" "$WORKDIR/start.cc.log" "$LIBC_DIR/start.c" "$WORKDIR/start.s"
...
echo "Compiler (stage07): $STAGE7_CC"
...
# ============================================================
# Step 2: Build gen1_cc using stage07's s12cc (building stage08 compiler)
# ============================================================
echo ""
echo "=== Step 2: Build gen1_cc (compiled by stage07 s12cc) ==="
...
# Compile stage08 source with stage07 compiler
run_exe "$STAGE7_CC" "$WORKDIR/gen1_cc-compile.log" "$STAGE_CC_SRC" "$WORKDIR/gen1_cc.s"
...
if [[ -s "$GEN1_CC_EXE" ]]; then
    ...
    for tst in "$TESTS_DIR"/test_spike.c "$TESTS_DIR"/test_phase2.c "$TESTS_DIR"/test_phase3.c "$TESTS_DIR"/test_phase4.c "$TESTS_DIR"/test_phase5.c "$TESTS_DIR"/test_phase6.c "$TESTS_DIR"/test_phase7.c "$TESTS_DIR"/test_phase8.c "$TESTS_DIR"/test_phase9.c "$TESTS_DIR"/test_phase10.c "$TESTS_DIR"/test_phase11.c "$TESTS_DIR"/test_phase12.c "$TESTS_DIR"/test_phase13.c "$TESTS_DIR"/test_phase14.c "$TESTS_DIR"/test_phase15.c "$TESTS_DIR"/test_phase16.c "$TESTS_DIR"/test_phase17.c "$TESTS_DIR"/test_phase18.c "$TESTS_DIR"/test_phase19.c "$TESTS_DIR"/test_phase20.c "$TESTS_DIR"/test_phase21.c "$TESTS_DIR"/test_phase22.c "$TESTS_DIR"/test_phase23.c "$TESTS_DIR"/test_phase24.c "$TESTS_DIR"/test_phase25.c "$TESTS_DIR"/test_phase26.c "$TESTS_DIR"/test_phase27.c "$TESTS_DIR"/test_phase28.c "$TESTS_DIR"/test_phase29.c "$TESTS_DIR"/test_phase30.c "$TESTS_DIR"/test_phase31.c "$TESTS_DIR"/test_phase32.c "$TESTS_DIR"/test_phase33.c "$TESTS_DIR"/test_short.c "$TESTS_DIR"/test_bitfields.c "$TESTS_DIR"/test_many_args.c "$TESTS_DIR"/test_pp_predefs.c "$TESTS_DIR"/test_ret_widen.c "$TESTS_DIR"/test_libutf_bugs.c "$TESTS_DIR"/test_decl_list.c "$TESTS_DIR"/test_sqlite_bugs.c "$TESTS_DIR"/test_narrow_rmw.c "$TESTS_DIR"/test_tentative_init.c "$TESTS_DIR"/test_grouped_ptr_array.c "$TESTS_DIR"/test_const_shift.c "$TESTS_DIR"/test_pp_if_text.c "$TESTS_DIR"/test_addr_paren.c "$TESTS_DIR"/test_int_to_ptr_paren.c "$TESTS_DIR"/test_define_big.c "$TESTS_DIR"/test_string_concat.c "$TESTS_DIR"/test_duff_jt.c "$TESTS_DIR"/test_jt_fallthrough.c "$TESTS_DIR"/test_jt_goto_case.c "$TESTS_DIR"/test_pp_dis_args.c "$TESTS_DIR"/test_fnptr_local.c "$TESTS_DIR"/test_fp64_split.c "$TESTS_DIR"/test_fnptr_double.c "$TESTS_DIR"/test_inline.c "$TESTS_DIR"/test_dce_inline.c "$TESTS_DIR"/test_leaf_frame.c "$TESTS_DIR"/test_retpin.c "$TESTS_DIR"/test_bigframe_r30.c "$TESTS_DIR"/test_spill_slots.c "$TESTS_DIR"/test_block_slots.c "$TESTS_DIR"/test_inline_loop.c "$TESTS_DIR"/test_2d_init.c "$TESTS_DIR"/test_pp_param_in_string.c "$TESTS_DIR"/test_static_list.c "$TESTS_DIR"/test_row_param.c "$TESTS_DIR"/test_pp_octal.c "$TESTS_DIR"/test_big_agg.c "$TESTS_DIR"/test_clit_init.c; do
        ...
        if [[ "$tname" == "test_inline_loop" ]]; then
            # GitHub issue 73: loop/switch-bearing statics need budget 200 to splice.
            S12CC_INLINE=200 run_exe "$GEN1_CC_EXE" "$WORKDIR/${tname}-gen1_cc.log" $EXTRA_CC_ARGS "$tst" "$WORKDIR/${tname}.s"
        elif [[ "$tname" == "test_dce_inline" || "$tname" == "test_inline" ]]; then
        ...
        # GitHub issue 64: top-level fall-through still emits a jump table;
        # Duff's device (nested labels) must not.
        if [[ "$tname" == "test_jt_fallthrough" ]]; then
            if ! grep -qE '\.LJT' "$WORKDIR/${tname}.s"; then
                printf "  %-30s FAIL (no jump table)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        fi
        ...
        # sumarr's array keeps an SP-relative frame (no FP setup);
        # withcall saves lr but not the frame pointer.  r30 may appear
        # as a GPR on the omit-fp path.
        if [[ "$tname" == "test_leaf_frame" ]]; then
            ...
            if awk '/^sumarr:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'addi r30, r29'; then
                printf "  %-30s FAIL (sumarr still sets up r30 as fp)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
            ...
            if awk '/^withcall:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'addi r30, r29'; then
                printf "  %-30s FAIL (withcall still sets up r30 as fp)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        ...
        # GitHub issue 72: afterloop accumulator lives in r1 — no copy at the ret.
        # diamond's join phi is r1 (each arm writes r1, no copy at the join).
        ...
        # GitHub issue 73: the loop/switch statics must actually have been
        # spliced, or the test proves nothing.
        if [[ "$tname" == "test_inline_loop" ]]; then
            if grep -qE '^(another|deeper|classify):' "$WORKDIR/${tname}.s"; then
                printf "  %-30s FAIL (static not spliced at budget 200)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        fi
        ...
        if [[ "$tname" == "test_spill_slots" ]]; then
            ...
            if [[ -z "$fsz" || "$fsz" -gt 320 ]]; then
                printf "  %-30s FAIL (phases frame is ${fsz:-?} bytes; spill slots not shared)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        fi
        # GitHub issue 73: a >2047-byte frame that took the r30 color pops
        # through r3, not r1 (the return value).
        if [[ "$tname" == "test_bigframe_r30" ]]; then
            if ! awk '/^bigframe:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'add r29, r29, r3'; then
                printf "  %-30s FAIL (bigframe does not pop through r3)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        fi
        if [[ "$tname" == "test_retpin" ]]; then
            if awk '/^afterloop:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'addi r1, r[3-9], 0|addi r1, r1[0-9], 0|addi r1, r2[0-8], 0'; then
                printf "  %-30s FAIL (afterloop still copies to r1)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
            if ! awk '/^afterloop:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'add r1, r1,'; then
                printf "  %-30s FAIL (afterloop accumulator not in r1)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
            if awk '/^diamond:/{p=1;next} p&&/^[A-Za-z_]/{exit} p' "$WORKDIR/${tname}.s" | grep -qE 'addi r1, r[3-9], 0|addi r1, r1[0-9], 0|addi r1, r2[0-8], 0'; then
                printf "  %-30s FAIL (diamond still copies to r1)\n" "$tname:"
                FAIL=$((FAIL + 1))
                continue
            fi
        fi
...
# ============================================================
# Step 3b: Semantic equivalence smoke (stage07 vs stage08 cc)
# ============================================================
if [[ -s "$GEN1_CC_EXE" ]]; then
    echo ""
    echo "=== Step 3b: Semantic equivalence (stage07 vs gen1_cc) ==="
...
EOF
    ...
    for src in \
        ...
        EXE_A=$(compile_and_link "${name}-s4" "$src" "$STAGE7_CC" "$AS_EXE" "$LD_EXE") || {
            printf "  %-30s FAIL (stage07 build)\n" "${name}:"
            FAIL=$((FAIL + 1))
            continue
        }
        ...
        if [[ "$RC_A" -eq "$RC_B" ]]; then
            ...
            echo "    stage07 run log:" >&2
            tail -n 20 "$WORKDIR/${name}-s4.run.log" >&2 || true
            echo "    stage08 run log:" >&2
...
# ============================================================
# Step 3c: Optimizer regression pack (stage07 vs stage08 cc)
# ============================================================
if [[ -s "$GEN1_CC_EXE" ]]; then
    echo ""
...
# ============================================================
# Step 4: Build tools with stage07's s12cc
# ============================================================
echo ""
echo "=== Step 4: Build stage08 tools ==="
...
# Use gen1_cc + stage08 tools to compile/assemble/link/run a program
# ============================================================
...
if [[ -s "$GEN1_CC_EXE" && -s "$STAGE_AS_EXE" && -s "$STAGE_LD_EXE" ]]; then
    echo ""
    echo "=== Step 5: End-to-end toolchain test (gen1_cc + stage08 tools) ==="

    # Test: compile test_spike.c with gen1_cc, assemble with stage08 AS, link with stage08 LD
    ...
    # Test: use stage08 archiver to create a libc archive, then link with it
    if [[ -s "$STAGE_AR_EXE" ]]; then
        ...
        if [[ ! -s "$WORKDIR/e2e-libc.s32a" ]]; then
            ...
            # Compile test_spike with gen1_cc, assemble with stage08 AS, link with archive
            ...
            run_exe "$STAGE_LD_EXE" "$WORKDIR/e2e-ar-ld.log" \
                ...
                $BUILTINS64_OBJ $BUILTINS_FP64_OBJ "$WORKDIR/e2e-libc.s32a"
...
echo ""
if [[ "$FAIL" -eq 0 ]]; then
    echo "OK: stage08 ($PASS/$TOTAL tests passed)"
else
    echo "FAIL: stage08 ($PASS/$TOTAL tests passed, $FAIL failed)" >&2
    exit 1
fi
