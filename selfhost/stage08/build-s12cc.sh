...
# Build cc.s32x: the stage08 compiler.
# Uses: Stage 07 cc.s32x (compiler), Stage 07 s32-as.s32x (assembler),
#       Stage 07 s32-ld.s32x (linker).
#
# Two-phase build:
#   Phase 1: stage07 compiles s12cc.c + libc → gen1 (tree-walk ABI)
#   Phase 2: gen1 recompiles libc → stage08 libc (HIR/SSA ABI, r11-r28 callee-saved)
#
...
# libc to avoid register clobbering (stage07 bootstrap libc may differ).
...
EMU="${SELFHOST_EMU:-}"
if [[ -z "$EMU" ]]; then
    # Stage08 policy (and the reason for the strict rule):
    # Stage00 deliberately symlinks s32-emu → slow32-dbt (when present) as the
    # single point of control for the entire bootstrap chain.  Therefore stage08
    # scripts must not do their own auto-detection of dbt or slow32-fast.
    # Respect SELFHOST_EMU for explicit override; otherwise just use whatever
    # stage00/s32-emu points at.
...
for f in "$EMU" "$STAGE7_CC" "$STAGE7_AS" "$STAGE7_LD" \
...
assemble() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE7_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE7_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
