...
choose_default_emu() {
    ...
    # stage00 emulator.  No automatic preference for the DBT.
...
Compare LLVM vs selfhost stage08 assembly shape.

Options:
  --clang <path>     Clang binary (default: \$LLVM_BIN/clang)
  --llvm-bin <dir>   LLVM bin dir (default: ~/llvm-project/build/bin)
  --self-cc <path>   Selfhost compiler exe (default: selfhost/stage08/cc.s32x)
  --emu <path>       Emulator for selfhost compiler (stage08 policy: SELFHOST_EMU or stage00/s32-emu only)
...
If no test files are passed, defaults to:
  selfhost/stage08/tests/codegen_compare.list
USAGE
}
...
for t in "${TESTS[@]}"; do
    ...
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$SELF_CC" "$t" "$self_asm" >/dev/null 2>&1 || {
