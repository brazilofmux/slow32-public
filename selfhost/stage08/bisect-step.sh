...
#   selfhost/stage08/bisect-step.sh --disable-fusion ; echo "exit=$?"
...
#   cp selfhost/stage08/bisect-step.sh /tmp/bisect-step.sh
#   git checkout main
#   git bisect start
#   git bisect bad HEAD
...
# Stage08 policy: respect SELFHOST_EMU, otherwise use the minimal trusted emulator.
EMU="${SELFHOST_EMU:-$ROOT_DIR/selfhost/stage00/s32-emu}"
RUN_TESTS="$ROOT_DIR/selfhost/stage08/run-tests.sh"
...
TEST_SRC="$ROOT_DIR/selfhost/stage08/diff-corpus/01_return_const.c"
if [[ ! -f "$TEST_SRC" ]]; then
...
BURG_FILE="$ROOT_DIR/selfhost/stage08/hir_burg.h"
PATCH_BACKUP=""
cleanup() {
...
WD_BEFORE="$(ls -dt /tmp/selfhost-v2-stage08.* 2>/dev/null | head -1 || true)"
...
WD="$(ls -dt /tmp/selfhost-v2-stage08.* 2>/dev/null | head -1 || true)"
