...
# Build cc.s32x: the stage07 compiler.
# Uses: Stage 06 cc.s32x (compiler), Stage 06 s32-as.s32x (assembler),
#       Stage 06 s32-ld.s32x (linker).
#
# Two-phase build:
#   Phase 1: stage06 compiles s12cc.c + libc → gen1 (tree-walk ABI)
#   Phase 2: gen1 recompiles libc → stage07 libc (HIR/SSA ABI, r11-r28 callee-saved)
#
...
# libc to avoid register clobbering (stage06 bootstrap libc may differ).
...
for f in "$EMU" "$STAGE6_CC" "$STAGE6_AS" "$STAGE6_LD" \
...
compile() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE6_CC" "$src" "$asm" >"$log" 2>&1
    local rc=$?
    set -e
...
assemble() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE6_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE6_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
# --- Build libc (compiled by stage06 s12cc) ---
echo "[2/4] Build libc"
LIBC_OBJS=""
...
# --- Compile compiler with stage06 compiler ---
