...
#
# Tests:
#   1. Build gen1_cc using stage06's s12cc (compiler bootstrap)
#   2. Run compiler tests with gen1_cc
#   3. Build tools (s32-as, s32-ar, s32-ld) using stage06's s12cc
#   4. End-to-end: gen1_cc + stage07 tools compile/assemble/link/run a program
...
Stage compiler tests: s12cc compiler + toolchain tests (bootstrapped from stage06)
USAGE
}
...
# ============================================================
# Step 1: Bootstrap — build runtime/libc using stage06 compiler
# ============================================================
echo "=== Step 1: Bootstrap ==="

STAGE6_CC="$SELFHOST_DIR/stage06/cc.s32x"
AS_EXE="$SELFHOST_DIR/stage06/s32-as.s32x"
LD_EXE="$SELFHOST_DIR/stage06/s32-ld.s32x"

[[ -f "$STAGE6_CC" ]] || { echo "Missing compiler (stage06): $STAGE6_CC" >&2; exit 1; }
...
# Build libc with stage06 compiler
LIBC_OBJS=""
for name in string_extra string_more ctype convert stdio malloc; do
    run_exe "$STAGE6_CC" "$WORKDIR/${name}.cc.log" "$LIBC_DIR/${name}.c" "$WORKDIR/${name}.s"
...
run_exe "$STAGE6_CC" "$WORKDIR/start.cc.log" "$LIBC_DIR/start.c" "$WORKDIR/start.s"
...
STAGE6_LIBC_OBJS="$LIBC_OBJS"
STAGE6_LIBC_START_OBJ="$LIBC_START_OBJ"

echo "Compiler (stage06): $STAGE6_CC"
...
# ============================================================
# Step 2: Build gen1_cc using stage06's s12cc (building stage07 compiler)
# ============================================================
echo ""
echo "=== Step 2: Build gen1_cc (compiled by stage06 s12cc) ==="
...
# Compile stage07 source with stage06 compiler
run_exe "$STAGE6_CC" "$WORKDIR/gen1_cc-compile.log" "$STAGE_CC_SRC" "$WORKDIR/gen1_cc.s"
...
if [[ -s "$GEN1_CC_EXE" ]]; then
    ...
    for name in string_extra string_more ctype convert stdio malloc printf_varargs; do
        ...
        if [[ ! -s "$WORKDIR/g1_${name}.s" ]]; then
            echo "  WARN: gen1 failed to compile ${name}.c, falling back to stage06 libc" >&2
            G1_LIBC_OBJS=""
            break
        fi
...
# ============================================================
# Step 3b: Semantic equivalence smoke (stage06 vs stage07 cc)
# ============================================================
if [[ -s "$GEN1_CC_EXE" ]]; then
    echo ""
    echo "=== Step 3b: Semantic equivalence (stage06 vs gen1_cc) ==="
...
EOF
    ...
    for src in \
        ...
        EXE_A=$(compile_and_link "${name}-s4" "$src" "$STAGE6_CC" "$AS_EXE" "$LD_EXE") || {
            printf "  %-30s FAIL (stage06 build)\n" "${name}:"
            FAIL=$((FAIL + 1))
            continue
        }
...
# ============================================================
# Step 3c: Optimizer regression pack (stage06 vs stage07 cc)
# ============================================================
if [[ -s "$GEN1_CC_EXE" ]]; then
    echo ""
...
# ============================================================
# Step 4: Build tools with stage06's s12cc
# ============================================================
echo ""
echo "=== Step 4: Build stage07 tools ==="
...
# Build assembler
TOTAL=$((TOTAL + 1))
run_exe "$STAGE6_CC" "$WORKDIR/s32-as-compile.log" "$STAGE_AS_SRC" "$WORKDIR/s32-as.s"
...
# Build archiver
TOTAL=$((TOTAL + 1))
run_exe "$STAGE6_CC" "$WORKDIR/s32-ar-compile.log" "$STAGE_AR_SRC" "$WORKDIR/s32-ar.s"
...
# Build linker
TOTAL=$((TOTAL + 1))
run_exe "$STAGE6_CC" "$WORKDIR/s32-ld-compile.log" "$STAGE_LD_SRC" "$WORKDIR/s32-ld.s"
...
# Use gen1_cc + stage07 tools to compile/assemble/link/run a program
# ============================================================
...
if [[ -s "$GEN1_CC_EXE" && -s "$STAGE_AS_EXE" && -s "$STAGE_LD_EXE" ]]; then
    echo ""
    echo "=== Step 5: End-to-end toolchain test (gen1_cc + stage07 tools) ==="

    # Test: compile test_spike.c with gen1_cc, assemble with stage07 AS, link with stage07 LD
    ...
    # Test: use stage07 archiver to create a libc archive, then link with it
    if [[ -s "$STAGE_AR_EXE" ]]; then
        ...
        if [[ ! -s "$WORKDIR/e2e-libc.s32a" ]]; then
            ...
            # Compile test_spike with gen1_cc, assemble with stage07 AS, link with archive
...
echo ""
if [[ "$FAIL" -eq 0 ]]; then
    echo "OK: stage07 ($PASS/$TOTAL tests passed)"
else
    echo "FAIL: stage07 ($PASS/$TOTAL tests passed, $FAIL failed)" >&2
    exit 1
fi
