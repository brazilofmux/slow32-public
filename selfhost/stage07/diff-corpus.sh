...
#   selfhost/stage07/run-tests.sh --fixed-point --keep-artifacts
#   WD=$(ls -dt /tmp/selfhost-v2-stage07.* | head -1)
#
#   # Iterate on hypotheses against the corpus (fast path).
#   selfhost/stage07/diff-corpus.sh --test "$WD/fp-gen2.s32x"
#
#   # Or compare two binaries directly:
#   selfhost/stage07/diff-corpus.sh \
#       --ref selfhost/stage07/cc.s32x \
...
Options:
  --ref <path>       Reference compiler (default: selfhost/stage07/cc.s32x)
  --test <path>      Suspect compiler (REQUIRED — no default)
  --emu <path>       Emulator (default: tools/dbt/slow32-dbt, then stage00/s32-emu)
  --corpus <dir>     Corpus directory (default: selfhost/stage07/diff-corpus)
...
if [[ -z "$EMU" ]]; then
    EMU="$(choose_default_emu)" || {
        echo "No emulator found (looked for tools/dbt/slow32-dbt and stage00/s32-emu)" >&2
        exit 2
    }
fi
...
if [[ -z "$TEST_CC" ]]; then
    ...
    echo "Hint: pass a path like /tmp/selfhost-v2-stage07.*/fp-gen2.s32x" >&2
    exit 2
fi
