...
int parse_obj_header() {
    ...
    if (obj_nsec < 0 || obj_nsec > (FILE_BUFSZ / S32O_SEC_SZ) ||
        ...
        fdputs("error: malformed object tables\n", 2);
        return 0;
    }
            ...
            if (stype != SEC_BSS) {
                if (ssize > 0 && sfileoff > 0) {
                    ...
                    if (!in_bounds(sfileoff, ssize, obj_file_sz)) {
                        fdputs("error: section data out of range\n", 2);
                        link_error = 1;
                        return;
                    }
...
void merge_symbols() {
    ...
    while (i < obj_nsym) {
        ...
        if (sy_noff < 0 || sy_noff >= obj_str_sz) {
            fdputs("error: symbol name offset out of range\n", 2);
            link_error = 1;
            return;
        }
        ...
        if (name_len >= obj_str_sz - sy_noff) {
            fdputs("error: unterminated symbol name\n", 2);
            link_error = 1;
            return;
        }
        ...
        if (sy_sec == 0) {
            ...
            if (sy_sec < 1 || sy_sec > obj_nsec) {
                fdputs("error: symbol section index out of range\n", 2);
                link_error = 1;
                return;
            }
            ...
            if (sy_sectype != SEC_CODE && sy_sectype != SEC_DATA &&
                ...
                fdputs("error: symbol section type invalid\n", 2);
                link_error = 1;
                return;
            }
...
void merge_relocs() {
    ...
    while (i < obj_nsec) {
        ...
        if (nrelocs < 0 || nrelocs > (obj_file_sz / S32O_REL_SZ)) {
            fdputs("error: relocation table out of range\n", 2);
            link_error = 1;
            return;
        }
...
void load_ar_member(int ar_fh, int midx) {
    ...
    if (!in_bounds(mem_off, mem_sz, ar_file_sz)) {
        fdputs("error: archive member range invalid\n", 2);
        link_error = 1;
        return;
    }
...
void link_archive(char *path) {
    ...
    if (fdseek(ar_fh, 0, SEEK_END) != 0) {
        fdputs("error: cannot size archive\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ar_end = fdtell(ar_fh);
    if (ar_end < 32) {
        fdputs("error: invalid archive size\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ar_file_sz = ar_end;
    if (fdseek(ar_fh, 0, SEEK_SET) != 0) {
        fdputs("error: cannot rewind archive\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ...
    if (!in_bounds(ar_sym_off, ar_nsymbols * 8, ar_file_sz) ||
        ...
        fdputs("error: archive table offsets out of range\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ...
    if (fdread(ar_symtab, 1, ar_nsymbols * 8, ar_fh) != ar_nsymbols * 8) {
        fdputs("error: short read on archive symtab\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ...
    if (fdread(ar_memtab, 1, ar_nmembers * 24, ar_fh) != ar_nmembers * 24) {
        fdputs("error: short read on archive member table\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
    ...
    if (fdread(ar_strtab, 1, ar_str_sz, ar_fh) != ar_str_sz) {
        fdputs("error: short read on archive string table\n", 2);
        fdclose(ar_fh);
        link_error = 1;
        return;
    }
...
void apply_rel(int idx) {
    ...
    if (r_typ == REL_32 || r_typ == REL_ABS32) {
        ...
        if ((offset & 1) != 0 || offset < -4096 || offset > 4094) {
            fdputs("error: branch relocation out of range\n", 2);
            link_error = 1;
            return;
        }
        imm12 = (offset >> 12) & 1;
        ...
        if ((offset & 1) != 0 || offset < -1048576 || offset > 1048574) {
            fdputs("error: jal relocation out of range\n", 2);
            link_error = 1;
            return;
        }
        imm20 = (offset >> 20) & 1;
