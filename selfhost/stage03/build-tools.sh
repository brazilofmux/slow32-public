...
# Build stage03 tools: s32-as.s32x, s32-ar.s32x, s32-ld.s32x
# All compiled by stage03's cc.s32x, assembled by stage02 assembler,
# linked by stage02 linker. Uses stage03's own libc/runtime.
...
WORKDIR="$(mktemp -d /tmp/stage03-tools.XXXXXX)"
...
compile() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE3_CC" "$src" "$asm" >"$log" 2>&1
    local rc=$?
    set -e
...
assemble() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE2_AS" "$src" "$obj" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
...
link_exe() {
    ...
    shift
    set +e
    timeout "${SELFHOST_TIMEOUT:-1200}" "$EMU" "$STAGE2_LD" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -ne 0 && "$rc" -ne 96 ]]; then
