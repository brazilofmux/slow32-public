...
static void p_program(void) {
    int bt; int pt; int rt; char fn[128]; int sci; int scv;
    ...
    while (lex_tok != TK_EOF) {
        ...
        sci = 0;
        if (lex_tok == TK_STATIC) { next(); sci = 1; }
        if (lex_tok == TK_EXTERN) next();
        if (sci && lex_tok == TK_CONST) {
            next();
            if (lex_tok == TK_INT) {
                next();
                if (lex_tok == TK_IDENT) {
                    ...
                    if (lex_tok == TK_ASSIGN) {
                        next(); scv = 0;
                        if (lex_tok == TK_MINUS) { next(); scv = 0 - lex_val; next(); }
                        else { scv = lex_val; next(); }
                        p_adf(fn, scv); p_expect(TK_SEMI); continue;
                    }
                }
            }
        }
