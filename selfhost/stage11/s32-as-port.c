...
static int g_lbl_name_off[MAX_LBL];
static int g_lbl_name_ptr;
static int g_lbl_sec[MAX_LBL];
static int g_lbl_val[MAX_LBL];
static char g_lbl_defd[MAX_LBL];
static char g_lbl_glob[MAX_LBL];
static char g_lbl_refd[MAX_LBL];
static int g_nlbl;

static int g_rel_sec[MAX_REL];
static int g_rel_off[MAX_REL];
static int g_rel_typ[MAX_REL];
static int g_rel_sym[MAX_REL];
static int g_rel_add[MAX_REL];
static int g_nrel;

static char g_text[MAX_TEXT];
static char g_data[MAX_DATA];
static int g_tsz;
static int g_dsz;
static int g_bsz;
static int g_sec;

static int g_lbl_to_sym[MAX_LBL];
static int g_nsym;
static int g_sym_lbl[MAX_SYM];
static int g_sym_name[MAX_SYM];
static int g_sym_val[MAX_SYM];
static int g_sym_sec[MAX_SYM];
static char g_sym_bind[MAX_SYM];
...
int get_lbl(char *name) {
    ...
    n = strlen(name) + 1;
    if (g_lbl_name_ptr + n > 131072) return -1;
...
int write_obj(char *out) {
    ...
    off = rel_off;
    if (text_rel) { text_rel_off = off; off += text_rel * SIZEOF_S32O_RELOC; }
    if (data_rel) { data_rel_off = off; off += data_rel * SIZEOF_S32O_RELOC; }
    if (bss_rel) { bss_rel_off = off; off += bss_rel * SIZEOF_S32O_RELOC; }
    str_off = off;
    off += g_ssz;
    off = (off + 3) & -4;
