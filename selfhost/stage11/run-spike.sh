...
# Stage 11: s32-cc C compiler (bootstrapped from stage10)
#
# Bootstrap chain:
# 1) Bootstrap via stage10 s32-cc (compiler, assembler, linker, runtime)
# 2) Lexer tests: compile each test_lex_*.c with s32-cc, run
# 3) Build s32-cc: concatenate lex+parse headers, compile s32cc.c with s32-cc
...
echo "Bootstrap compiler (stage10): $GEN2_EXE"
