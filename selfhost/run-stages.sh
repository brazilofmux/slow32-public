...
run_stage00() {
    ...
    make -C "$ROOT_DIR/selfhost/stage00" >/tmp/v2-stage00-build.log 2>&1
    make -C "$ROOT_DIR/selfhost/stage00" test >/tmp/v2-stage00-test.log 2>&1
}

run_stage01() {
    echo "[stage01] assembler regression"
    STAGE01_EMU="$EMU" "$ROOT_DIR/selfhost/stage01/run-regression.sh" test1 >/tmp/v2-stage01.log 2>&1
}

run_stage02() {
    echo "[stage02] archiver regression"
    STAGE2_AR_EMU="$EMU" "$ROOT_DIR/selfhost/stage02/run-regression.sh" test3 >/tmp/v2-stage02.log 2>&1
}

run_stage03() {
    echo "[stage03] linker regression"
    STAGE3_EMU="$EMU" "$ROOT_DIR/selfhost/stage03/run-regression.sh" test3 >/tmp/v2-stage03-test3.log 2>&1
    STAGE3_EMU="$EMU" "$ROOT_DIR/selfhost/stage03/run-regression.sh" archive >/tmp/v2-stage03-archive.log 2>&1
    if [[ "$SKIP_SELFHOST_KERNEL" -eq 0 ]]; then
        SELFHOST_EMU="$EMU" "$ROOT_DIR/selfhost/stage03/run-selfhost-kernel.sh" >/tmp/v2-stage03-selfhost-kernel.log 2>&1
    else
...
run_stage04() {
    ...
    STAGE4_EMU="$EMU" "$ROOT_DIR/selfhost/stage04/run-regression.sh" >/tmp/v2-stage04.log 2>&1
    echo "[stage04] subset-c conformance"
    STAGE4_EMU="$EMU" "$ROOT_DIR/selfhost/stage04/run-subset-conformance.sh" >/tmp/v2-stage04-subset.log 2>&1
    echo "[stage04] stage5-idiom conformance"
    STAGE4_EMU="$EMU" "$ROOT_DIR/selfhost/stage04/run-subset-conformance.sh" \
...
run_stage05() {
    ...
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode progressive-as --test test1 --emu "$EMU" >/tmp/v2-stage05-test1.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode progressive-as --test test2 --emu "$EMU" >/tmp/v2-stage05-test2.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode progressive-as --test test3 --emu "$EMU" >/tmp/v2-stage05-test3.log 2>&1
}

run_stage06() {
    echo "[stage06] c-archiver replacement smoke (c/rc/t/x/d/cs)"
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode stage6-ar-smoke --emu "$EMU" >/tmp/v2-stage06-smoke.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode stage6-ar-rc-smoke --emu "$EMU" >/tmp/v2-stage06-rc.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode stage6-ar-tx-smoke --emu "$EMU" >/tmp/v2-stage06-tx.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode stage6-ar-d-smoke --emu "$EMU" >/tmp/v2-stage06-d.log 2>&1
    "$ROOT_DIR/selfhost/stage05/run-pipeline.sh" --mode stage6-ar-scan-smoke --emu "$EMU" >/tmp/v2-stage06-scan.log 2>&1
}

run_stage07() {
    echo "[stage07] c-linker replacement spike (+ reloc)"
    "$ROOT_DIR/selfhost/stage07/run-spike.sh" --emu "$EMU" --with-reloc-spike >/tmp/v2-stage07.log 2>&1
}

run_stage08() {
    echo "[stage08] pragmatic c-archiver parity gate"
    "$ROOT_DIR/selfhost/stage08/run-regression.sh" --emu "$EMU" >/tmp/v2-stage08.log 2>&1
}
