...
# Stage 13: New compiler (copy of stage12, bootstrapped by stage12)
#
# Bootstrap chain:
#   stage12/s12cc → compile stage13 sources
...
Stage13 tests: s12cc compiler tests (bootstrapped from stage12)
USAGE
}
...
S12CC_EXE="$SELFHOST_DIR/stage12/s12cc.s32x"
[[ -f "$S12CC_EXE" ]] || { echo "Missing s12cc (stage12): $S12CC_EXE" >&2; exit 1; }
...
run_exe "$S12CC_EXE" "$WORKDIR/s13cc-compile.log" "$S13CC_SRC" "$WORKDIR/s13cc.s"
...
if [[ -s "$S13CC_EXE" ]]; then
    ...
    for tst in "$TESTS_DIR"/test_spike.c "$TESTS_DIR"/test_phase2.c "$TESTS_DIR"/test_phase3.c "$TESTS_DIR"/test_phase4.c "$TESTS_DIR"/test_phase5.c "$TESTS_DIR"/test_phase6.c "$TESTS_DIR"/test_phase7.c "$TESTS_DIR"/test_phase8.c "$TESTS_DIR"/test_phase9.c "$TESTS_DIR"/test_phase10.c "$TESTS_DIR"/test_phase11.c "$TESTS_DIR"/test_phase12.c; do
        ...
        # Compile with s13cc
        run_exe "$CC_EXE" "$WORKDIR/${tname}-s13cc.log" "$tst" "$WORKDIR/${tname}.s"
