...
: PARSE-REG ( addr u -- regnum true | false )
    DUP 0= IF 2DROP FALSE EXIT THEN
    \ Check aliases (case insensitive) — strict subset of tools/assembler
...
\ not rd, rs = xori rd, rs, -1 (opcode 30)
: DO-NOT
    GET-TOK PARSE-REG 0= IF DROP S" bad rd" ASM-ERR EXIT THEN >R
    GET-TOK PARSE-REG 0= IF DROP S" bad rs" ASM-ERR R> DROP EXIT THEN
    30 R> ROT -1 ENC-I EMIT-W32 ;
...
: TRY-DIR ( addr u -- handled? )
    ...
    2DUP S" .long"    STREQI IF 2DROP DO-WORD    TRUE EXIT THEN
    ...
    2DUP S" .ident"   STREQI IF 2DROP DO-FILE    TRUE EXIT THEN
