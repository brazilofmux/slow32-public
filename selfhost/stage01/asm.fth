...
: PARSE-REG ( addr u -- regnum true | false )
    ...
    2DUP S" rv"   STREQI IF 2DROP  1 TRUE EXIT THEN
    2DUP S" t0"   STREQI IF 2DROP  2 TRUE EXIT THEN
    2DUP S" a0"   STREQI IF 2DROP  3 TRUE EXIT THEN
    2DUP S" a1"   STREQI IF 2DROP  4 TRUE EXIT THEN
    2DUP S" a2"   STREQI IF 2DROP  5 TRUE EXIT THEN
    2DUP S" a3"   STREQI IF 2DROP  6 TRUE EXIT THEN
    2DUP S" a4"   STREQI IF 2DROP  7 TRUE EXIT THEN
    2DUP S" a5"   STREQI IF 2DROP  8 TRUE EXIT THEN
    2DUP S" a6"   STREQI IF 2DROP  9 TRUE EXIT THEN
    2DUP S" a7"   STREQI IF 2DROP 10 TRUE EXIT THEN
    2DUP S" s0"   STREQI IF 2DROP 11 TRUE EXIT THEN
    2DUP S" s1"   STREQI IF 2DROP 12 TRUE EXIT THEN
    2DUP S" s2"   STREQI IF 2DROP 13 TRUE EXIT THEN
    2DUP S" s3"   STREQI IF 2DROP 14 TRUE EXIT THEN
    2DUP S" s4"   STREQI IF 2DROP 15 TRUE EXIT THEN
    2DUP S" s5"   STREQI IF 2DROP 16 TRUE EXIT THEN
    2DUP S" s6"   STREQI IF 2DROP 17 TRUE EXIT THEN
    2DUP S" s7"   STREQI IF 2DROP 18 TRUE EXIT THEN
    2DUP S" s8"   STREQI IF 2DROP 19 TRUE EXIT THEN
    2DUP S" s9"   STREQI IF 2DROP 20 TRUE EXIT THEN
    2DUP S" s10"  STREQI IF 2DROP 21 TRUE EXIT THEN
    2DUP S" s11"  STREQI IF 2DROP 22 TRUE EXIT THEN
    2DUP S" s12"  STREQI IF 2DROP 23 TRUE EXIT THEN
    2DUP S" s13"  STREQI IF 2DROP 24 TRUE EXIT THEN
    2DUP S" s14"  STREQI IF 2DROP 25 TRUE EXIT THEN
    2DUP S" s15"  STREQI IF 2DROP 26 TRUE EXIT THEN
    2DUP S" s16"  STREQI IF 2DROP 27 TRUE EXIT THEN
    2DUP S" s17"  STREQI IF 2DROP 28 TRUE EXIT THEN
...
\ seqz rd, rs = seq rd, rs, r0 (opcode 14)
: DO-SEQZ
    GET-TOK PARSE-REG 0= IF DROP S" bad rd" ASM-ERR EXIT THEN >R
    GET-TOK PARSE-REG 0= IF DROP S" bad rs" ASM-ERR R> DROP EXIT THEN
    14 R> ROT 0 ENC-R EMIT-W32 ;

\ snez rd, rs = sne rd, rs, r0 (opcode 15)
: DO-SNEZ
    GET-TOK PARSE-REG 0= IF DROP S" bad rd" ASM-ERR EXIT THEN >R
    GET-TOK PARSE-REG 0= IF DROP S" bad rs" ASM-ERR R> DROP EXIT THEN
    15 R> ROT 0 ENC-R EMIT-W32 ;
...
: TRY-INSN ( addr u -- handled? )
    ...
    \ Load aliases supported by official assembler
    ...
    2DUP S" tail"  STREQI IF 2DROP DO-TAIL  TRUE EXIT THEN
    2DUP S" neg"   STREQI IF 2DROP DO-NEG   TRUE EXIT THEN
    2DUP S" not"   STREQI IF 2DROP DO-NOT   TRUE EXIT THEN
    2DUP S" seqz"  STREQI IF 2DROP DO-SEQZ  TRUE EXIT THEN
    2DUP S" snez"  STREQI IF 2DROP DO-SNEZ  TRUE EXIT THEN
