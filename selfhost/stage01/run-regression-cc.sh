...
TEST_DIR="${STAGE01_CC_TEST_DIR:-$SCRIPT_DIR/tests/conformance/baseline}"
...
Runs Stage 01 compiler regression in a staged work-up:
  ...
  Stage D (optional): validation/slow32dump.c
    - on minimal emulator (selfhost/stage00/s32-emu): smoke-run (--help)
...
run_exe() {
    ...
    set +e
    timeout "${SELFHOST_TIMEOUT:-30}" "$EMU" "$exe" "$@" >"$log" 2>&1
    local rc=$?
    set -e
    if [[ "$rc" -eq 124 ]]; then
