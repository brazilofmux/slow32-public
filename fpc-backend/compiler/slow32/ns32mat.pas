{
    ...
    Generate SLOW-32 assembler for math nodes
...
interface
    ...
    type
      ts32moddivnode = class(tcgmoddivnode)
      ...
      ts32shlshrnode = class(tcgshlshrnode)
...
implementation
            ...
            case left.location.loc of
              ...
              LOC_SUBSETREF, LOC_CSUBSETREF:
                begin
                  ...
                  { SEQ dst, src, r0: sets dst=1 if src==0, i.e. boolean NOT }
    ...
    procedure ts32moddivnode.emit_div_reg_reg_reg(signed: boolean; denum, num, res: tregister);
      var
        op: TAsmOp;
      begin
        if signed then
    ...
    procedure ts32moddivnode.emit_mod_reg_reg_reg(signed: boolean; denum, num, res: tregister);
      var
        op: TAsmOp;
      begin
        if signed then
    ...
    function ts32shlshrnode.first_shlshr64bitint: tnode;
      begin
        result := nil;
      end;


    procedure ts32shlshrnode.second_64bit;
      var
        v : TConstExprInt;
          ...
          begin
            ...
            if nodetype = shln then
              begin
                ...
                { SLOW-32 has no A_BLTZ pseudo-op; use BLT reg,r0 equivalent }
                cg.a_cmp_const_reg_label(current_asmdata.CurrAsmList,OS_INT,OC_LT,0,tmpreg1,less32);
                emit_instr(taicpu.op_reg_const(A_LI,resreg.reglo,0));
...
begin
   cmoddivnode:=ts32moddivnode;
   cshlshrnode:=ts32shlshrnode;
   cunaryminusnode:=ts32unaryminusnode;
   cnotnode:=ts32notnode;
end.
