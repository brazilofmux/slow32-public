{
    ...
    SLOW-32 specific calling conventions
...
implementation
      ...
      begin
         case p.typ of
            orddef:
              result:=LOC_REGISTER;
            floatdef:
              { SLOW-32 has no FPU registers, all floats passed in GPRs }
              result:=LOC_REGISTER;
            enumdef:
              result:=LOC_REGISTER;
            pointerdef:
    ...
    function tcpuparamanager.get_volatile_registers_int(calloption: tproccalloption): tcpuregisterset;
      begin
        { Volatile (caller-saved): r1(rv), r2(t0), r3-r10(a0-a7)
          r0 is the zero register and is included but not meaningful.
          Non-volatile (callee-saved): r11-r28(s0-s17), r29(sp), r30(fp), r31(lr) }
        result:=[RS_R1..RS_R10];
      end;
    ...
    function tcpuparamanager.get_volatile_registers_fpu(calloption: tproccalloption): tcpuregisterset;
      begin
        { SLOW-32 has no FPU register file }
        result:=[];
      end;
    ...
    function tcpuparamanager.get_saved_registers_int(calloption : tproccalloption):tcpuregisterarray;
      const
        { Callee-saved: r29(sp), r30(fp), r11-r28(s0-s17) }
    ...
    function tcpuparamanager.get_saved_registers_fpu(calloption : tproccalloption):tcpuregisterarray;
      const
        { SLOW-32 has no FPU registers to save }
      ...
      begin
        ...
        with paraloc^ do
         begin
           ...
           def:=pdef;
           { SLOW-32: 8 argument registers r3-r10 (a0-a7) }
           if (nr<=8) then
             begin
               if nr=0 then
                 internalerror(2024121501);
               loc:=LOC_REGISTER;
               register:=newreg(R_INTREGISTER,RS_R3+nr-1,R_SUBWHOLE);
             end
           else
             begin
               loc:=LOC_REFERENCE;
    ...
    procedure tcpuparamanager.init_values(var curintreg, curfloatreg, curmmreg: tsuperregister; var cur_stack_offset: aword);
      begin
        cur_stack_offset := 0;
        { First argument register is r3 (a0) }
        curintreg := RS_R3;
        { SLOW-32 has no FPU or MM registers }
        curfloatreg := RS_NO;
        curmmreg := RS_NO;
      end;
      ...
      begin
        ...
        if push_addr_param(varspez, paradef, p.proccalloption) then
          ...
          begin
            ...
            { For SLOW-32, even if a record contains a single float field,
              we pass it in a GPR since there are no FPU registers }
            if (((paradef.typ=arraydef) and not
        ...
        { patch FPU values into integer registers if we are processing varargs
          (SLOW-32 always uses GPRs, but keep this for consistency) }
        ...
        while (paralen > 0) do begin
          ...
          if (loc = LOC_REGISTER) and
            ...
            begin
              { either LOC_REFERENCE, or LOC_REGISTER which must be passed on the
              ...
              if (side = callerside) then
                ...
                begin
                  { during procedure entry, NR_FRAME_POINTER_REG contains the old stack pointer }
                  ...
                  if assigned(current_procinfo) then
                    ts32procinfo(current_procinfo).needs_frame_pointer := true;
                end;
