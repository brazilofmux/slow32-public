{
    ...
    Generate SLOW-32 assembler for in set/case nodes
...
interface
    ...
    type
       ts32casenode = class(tcgcasenode)
         protected
...
*****************************************************************************}
    ...
    procedure ts32casenode.optimizevalues(var max_linear_list : int64; var max_dist : qword);
      begin
        max_linear_list:=3;
      end;


    function ts32casenode.has_jumptable : boolean;
      begin
        has_jumptable:=true;
      end;


    procedure ts32casenode.genjumptable(hp : pcaselabel;min_,max_ : int64);
      var
        table : tasmlabel;
        last : TConstExprInt;
        indexreg : tregister;
      ...
      begin
        ...
        { jump }
        current_asmdata.CurrAsmList.concat(taicpu.op_reg_reg(A_JALR,NR_R0, indexreg));
    ...
    procedure ts32casenode.genlinearlist(hp: pcaselabel);
      begin
        { a sub-based list makes no sense for SLOW-32 having no flags }
        genlinearcmplist(hp);
      end;


begin
   ccasenode:=ts32casenode;
end.
