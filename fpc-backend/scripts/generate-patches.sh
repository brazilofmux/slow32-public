...
git diff $BASE -- compiler/Makefile compiler/Makefile.fpc > "$PATCHES_DIR/01-compiler-build.patch" 2>/dev/null || true
...
git diff $BASE -- compiler/systems.inc compiler/systems.pas compiler/entfile.pas > "$PATCHES_DIR/02-system-registration.patch" 2>/dev/null || true
...
git diff $BASE -- compiler/fpcdefs.inc compiler/globals.pas compiler/options.pas compiler/version.pas compiler/cgbase.pas > "$PATCHES_DIR/03-compiler-defines.patch" 2>/dev/null || true
...
git diff $BASE -- compiler/systems/i_embed.pas compiler/systems/t_embed.pas > "$PATCHES_DIR/04-target-registration.patch" 2>/dev/null || true
...
git diff $BASE -- rtl/inc/systemh.inc rtl/inc/system.inc > "$PATCHES_DIR/07-rtl-system.patch" 2>/dev/null || true
