...
void putchar(int c) {
    // Use a fixed location in data buffer for single chars
    DATA_BUFFER[0] = (uint8_t)c;
    mmio_request(OP_PUTCHAR, 1, 0, 1);  // fd=1 (stdout)
}
...
void puts(const char *s) {
    ...
    uint32_t len = 0;
    while (s[len] && len < 1024) {  // Limit to 1KB
        DATA_BUFFER[len] = s[len];
        len++;
    }
    ...
    mmio_request(OP_WRITE, len, 0, 1);  // fd=1 (stdout)
}

// Write bytes to a file descriptor
int write(int fd, const void *buf, size_t count) {
    // Limit to what fits in data buffer
    ...
    return count;  // TODO: Get actual written count from response
}
