...
# Memory swap - swap contents of two memory regions in place (no temp buffer)
# Args: r3 = a, r4 = b, r5 = size (bytes)
# Word-optimized: swaps 4 bytes per iteration when both pointers are aligned
memswap:
    addi sp, sp, -32
    stw sp+0, lr
    stw sp+4, fp
    ...
    # Check if count == 0
    beq r13, r0, .memswap_done

    # Check alignment compatibility: (a ^ b) & 3
    xor r14, r11, r12
    andi r14, r14, 3
    bne r14, r0, .memswap_byte_loop

    # Phase 1: Align to 4-byte boundary
.memswap_align_loop:
    sgt r14, r13, r0
    beq r14, r0, .memswap_done
    andi r14, r11, 3
    beq r14, r0, .memswap_word_loop
    ldbu r14, r11+0
    ldbu r15, r12+0
    stb r11+0, r15
    stb r12+0, r14
    ...
    addi r12, r12, 1
    addi r13, r13, -1
    beq r0, r0, .memswap_align_loop

    # Phase 2: Word-wise swap
.memswap_word_loop:
    slti r14, r13, 4
    beq r14, r0, .memswap_word_body
    beq r0, r0, .memswap_tail_loop
