...
echo "=== Building Prolog Interpreter ==="
...
for src in src/*.c; do
    ...
    $CLANG $CFLAGS $OPT "$src" -o "src/$base.ll" 2>&1
    echo "OK"
...
# Link with MMIO libc
echo -n "  Linking... "
$LD --mmio 64K --stack-size 128K --data-size 5M -o prolog.s32x \
    ../runtime/crt0.s32o \
    $OBJECTS \
...
echo "  echo '?- write(hello), nl.' | ../tools/emulator/slow32-fast prolog.s32x"
