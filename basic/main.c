/*
** MY-BASIC shell adapted for SLOW-32
**
** Based on the MY-BASIC shell by Tony Wang
** (https://github.com/paladin-t/my_basic/)
** MIT License - Copyright (C) 2011 - 2025 Tony Wang
**
** Modifications for SLOW-32:
**  - Removed setjmp/longjmp (OOM -> abort)
**  - Removed atexit (explicit cleanup)
**  - Stubbed system() calls (CLS, DIR, SYS)
**  - Simplified ticks()/now() for SLOW-32 time API
**  - Removed strftime dependency
*/
...
#define _CHECK_MEM(__p) do { if(!(__p)) { printf("Error: out of memory.\n"); abort(); } } while(0)
...
static void _show_tip(void) {
	_printf("MY-BASIC Interpreter Shell - %s (SLOW-32)\n", mb_ver_string());
	...
	_printf("Input HELP and press enter to view the help information.\n");
}
...
#if _USE_MEM_POOL
	_printf("  -p n       - Set the memory pool threshold to `n` bytes\n");
#endif /* _USE_MEM_POOL */
	_printf("\n");
	...
	while(i < argc) {
		if(!memcmp(argv[i], "-", 1)) {
			if(!memcmp(argv[i] + 1, "e", 1)) {
				...
				_CHECK_ARG(argc, i, "-p: Memory pool threshold expected.\n");
				memp = argv[++i];
				if(argc > i + 1)
					prog = argv[++i];
#endif /* _USE_MEM_POOL */
...
static int raise_err(struct mb_interpreter_t* s, void** l) {
...
static void _on_startup(void) {
	...
	mb_register_func(bas, "RAISE", raise_err);
	mb_reg_fun(bas, gc);
	mb_reg_fun(bas, beep);
}
