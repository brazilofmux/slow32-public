...
- **Linux correspondence**: Opcodes map directly onto Linux/POSIX syscalls (`OPEN`, `READ`, `WRITE`, `SEEK`, `STAT`, `EXIT`). The emulator acts like the kernel: it interprets the descriptor/data buffer, performs the host OS call, and returns either positive results (bytes, offsets, fds) or negative errno codes in the completion descriptor.
