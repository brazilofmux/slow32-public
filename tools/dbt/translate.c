...
static void rc_flush(translate_ctx_t *ctx) {
    ...
    for (int i = 0; i < REG_ALLOC_SLOTS; i++) {
        if (ctx->reg_alloc[i].guest_reg >= 0 && ctx->reg_alloc[i].dirty) {
            emit_mov_m32_r32(&ctx->emit, RBP,
...
void translate_add(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    // LRU direct path: sources first, then destination
    x64_reg_t h1 = rc_read(ctx, rs1);
    x64_reg_t h2 = rc_read(ctx, rs2);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd == h1)       emit_add_r32_r32(e, hd, h2);       // rd==rs1: in-place
    else if (hd == h2)  emit_add_r32_r32(e, hd, h1);       // rd==rs2: commutative
    else { emit_mov_r32_r32(e, hd, h1); emit_add_r32_r32(e, hd, h2); }
}
...
void translate_sub(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    x64_reg_t h2 = rc_read(ctx, rs2);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd == h1)       emit_sub_r32_r32(e, hd, h2);       // rd==rs1: in-place
    else if (hd == h2) { emit_neg_r32(e, hd); emit_add_r32_r32(e, hd, h1); } // rd==rs2: NEG+ADD
    else { emit_mov_r32_r32(e, hd, h1); emit_sub_r32_r32(e, hd, h2); }
}
...
void translate_addi(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (imm != 0) emit_add_r32_imm32(e, RAX, imm);
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }
    ...
    x64_reg_t h1 = rc_read(ctx, rs1);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd != h1) emit_mov_r32_r32(e, hd, h1);
    if (imm != 0) emit_add_r32_imm32(e, hd, imm);
}
...
void translate_and(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    x64_reg_t h2 = rc_read(ctx, rs2);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd == h1)       emit_and_r32_r32(e, hd, h2);
    else if (hd == h2)  emit_and_r32_r32(e, hd, h1);       // Commutative
    else { emit_mov_r32_r32(e, hd, h1); emit_and_r32_r32(e, hd, h2); }
}
...
void translate_or(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    x64_reg_t h2 = rc_read(ctx, rs2);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd == h1)       emit_or_r32_r32(e, hd, h2);
    else if (hd == h2)  emit_or_r32_r32(e, hd, h1);        // Commutative
    else { emit_mov_r32_r32(e, hd, h1); emit_or_r32_r32(e, hd, h2); }
}
...
void translate_ori(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (imm != 0) emit_or_r32_imm32(e, RAX, imm);
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }
    ...
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd != h1) emit_mov_r32_r32(e, hd, h1);
    if (imm != 0) emit_or_r32_imm32(e, hd, imm);
}
...
void translate_xori(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (imm != 0) emit_xor_r32_imm32(e, RAX, imm);
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }
    ...
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd != h1) emit_mov_r32_r32(e, hd, h1);
    if (imm != 0) emit_xor_r32_imm32(e, hd, imm);
}
...
void translate_sll(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (rs2 != 0) { emit_load_guest_reg(ctx, RCX, rs2); emit_shl_r32_cl(e, RAX); }
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }

    x64_reg_t h1 = rc_read(ctx, rs1);
    x64_reg_t h2 = (rs2 != 0) ? rc_read(ctx, rs2) : (x64_reg_t)0;
    x64_reg_t hd = rc_write(ctx, rd);
    if (rs2 != 0) emit_mov_r32_r32(e, RCX, h2);
    if (hd != h1) emit_mov_r32_r32(e, hd, h1);
...
void translate_srl(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (rs2 != 0) { emit_load_guest_reg(ctx, RCX, rs2); emit_shr_r32_cl(e, RAX); }
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }
...
void translate_sra(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    if (!ctx->reg_cache_enabled) {
        emit_load_guest_reg(ctx, RAX, rs1);
        if (rs2 != 0) { emit_load_guest_reg(ctx, RCX, rs2); emit_sar_r32_cl(e, RAX); }
        emit_store_guest_reg(ctx, rd, RAX);
        return;
    }
...
static void emit_cmp_set_immediate(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2,
    ...
    if (ctx->reg_cache_enabled) {
        // LRU direct path: read sources from cache, compare, set result
        x64_reg_t h1 = rc_read(ctx, rs1);
        x64_reg_t h2 = rc_read(ctx, rs2);
        emit_cmp_r32_r32(e, h1, h2);
        // SETcc into RAX (safe scratch), then store to rd
        emit_setcc(e, RAX);
        emit_movzx_r32_r8(e, RAX, RAX);
        emit_store_guest_reg(ctx, rd, RAX);
    } else {
...
void translate_mul(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    // LRU: IMUL r32, r32 works with any registers; commutative
    x64_reg_t h1 = rc_read(ctx, rs1);
    x64_reg_t h2 = rc_read(ctx, rs2);
    x64_reg_t hd = rc_write(ctx, rd);
    if (hd == h1)       emit_imul_r32_r32(e, hd, h2);
    else if (hd == h2)  emit_imul_r32_r32(e, hd, h1);  // Commutative
    else { emit_mov_r32_r32(e, hd, h1); emit_imul_r32_r32(e, hd, h2); }
}
...
void translate_mulh(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t h1 = rc_read(ctx, rs1);
        x64_reg_t h2 = rc_read(ctx, rs2);
        emit_mov_r32_r32(e, RAX, h1);
        emit_imul_one_r32(e, h2);   // edx:eax = eax * h2 (signed)
        x64_reg_t hd = rc_write(ctx, rd);
        emit_mov_r32_r32(e, hd, RDX);  // High part
    } else {
        emit_load_guest_reg(ctx, RAX, rs1);
        emit_load_guest_reg(ctx, RCX, rs2);
        emit_imul_one_r32(e, RCX);
...
void translate_mulhu(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    if (ctx->reg_cache_enabled) {
        ...
        x64_reg_t h2 = rc_read(ctx, rs2);
        emit_mov_r32_r32(e, RAX, h1);
        emit_mul_r32(e, h2);   // edx:eax = eax * h2 (unsigned)
        x64_reg_t hd = rc_write(ctx, rd);
        emit_mov_r32_r32(e, hd, RDX);  // High part
    } else {
        emit_load_guest_reg(ctx, RAX, rs1);
...
void translate_div(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    ...
    // IDIV requires dividend in EDX:EAX and divisor in any register.
    // Use emit_load_guest_reg (handles r0 safely) for source loading.
    emit_load_guest_reg(ctx, RAX, rs1);
    emit_load_guest_reg(ctx, RCX, rs2);
...
void translate_ldw(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t hd = rc_write(ctx, rd);
        emit_mov_r32_m32_idx(e, hd, R14, addr);
    } else {
...
void translate_ldh(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t hd = rc_write(ctx, rd);
        emit_movsx_r32_m16_idx(e, hd, R14, addr);
    } else {
...
void translate_ldb(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t hd = rc_write(ctx, rd);
        emit_movsx_r32_m8_idx(e, hd, R14, addr);
    } else {
...
void translate_ldhu(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t hd = rc_write(ctx, rd);
        emit_movzx_r32_m16_idx(e, hd, R14, addr);
    } else {
...
void translate_ldbu(translate_ctx_t *ctx, uint8_t rd, uint8_t rs1, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        x64_reg_t hd = rc_write(ctx, rd);
        emit_movzx_r32_m8_idx(e, hd, R14, addr);
    } else {
...
void translate_stw(translate_ctx_t *ctx, uint8_t rs1, uint8_t rs2, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        if (rs2 == 0) {
            // r0 = 0: can't use rc_read(0) here because it XORs RAX, clobbering addr
            emit_xor_r32_r32(e, RCX, RCX);
            emit_mov_m32_r32_idx(e, R14, addr, RCX);
        } else {
            x64_reg_t h2 = rc_read(ctx, rs2);
            emit_mov_m32_r32_idx(e, R14, addr, h2);
        }
    } else {
        emit_load_guest_reg(ctx, RCX, rs2);
        emit_mov_m32_r32_idx(e, R14, addr, RCX);
...
void translate_sth(translate_ctx_t *ctx, uint8_t rs1, uint8_t rs2, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        if (rs2 == 0) {
            ...
            x64_reg_t h2 = rc_read(ctx, rs2);
            emit_mov_m16_r16_idx(e, R14, addr, h2);
        }
    } else {
        emit_load_guest_reg(ctx, RCX, rs2);
        emit_mov_m16_r16_idx(e, R14, addr, RCX);
...
void translate_stb(translate_ctx_t *ctx, uint8_t rs1, uint8_t rs2, int32_t imm) {
    ...
    if (ctx->reg_cache_enabled) {
        if (rs2 == 0) {
            ...
            x64_reg_t h2 = rc_read(ctx, rs2);
            emit_mov_m8_r8_idx(e, R14, addr, h2);
        }
    } else {
        emit_load_guest_reg(ctx, RCX, rs2);
        emit_mov_m8_r8_idx(e, R14, addr, RCX);
...
static size_t peephole_optimize_x64(uint8_t *code, size_t len, uint32_t guest_pc) {
    ...
    while (i + 6 < len) {
        ...
        // Pattern MOV_CMP disabled: the local deadness heuristic is unsound
        // across later path-specific uses of the moved register.
...
translated_block_t *translate_block_cached(translate_ctx_t *ctx, uint32_t guest_pc) {
    ...
    // emitter cannot overflow mid-block near the end of the buffer.
...
cached_block_done:
    ...
    // Code-buffer overflow guard: if the emitter ran past the remaining buffer,
    // host_size exceeds the buffer and the block is truncated. Do NOT peephole,
    // commit, or execute it. Flush once to reclaim the whole buffer and retry;
    // if it still doesn't fit, the block is larger than the entire buffer — bail.
    if (e->overflow) {
        if (!overflow_retry) {
            overflow_retry = true;
            cache_flush(cache);
