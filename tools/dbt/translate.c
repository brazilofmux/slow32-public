...
static uint32_t stage5_retry_budget_cfg_head(const stage5_lift_region_t *region) {
    if (!region || !region->cfg_valid || region->cfg_block_count <= 1) return 0;
    uint32_t budget = region->cfg_blocks[0].inst_count;
    if (budget < 2u || budget >= region->guest_inst_count) return 0;
    return budget;
}

static uint32_t stage5_retry_budget_half(const stage5_lift_region_t *region) {
    if (!region || region->guest_inst_count <= 3) return 0;
...
typedef enum {
    STAGE5_VALIDATE_MEM_OK = 0,
    STAGE5_VALIDATE_MEM_MMIO,
    STAGE5_VALIDATE_MEM_OOB,
    STAGE5_VALIDATE_MEM_CAPACITY
} stage5_validate_mem_status_t;

typedef struct {
    uint32_t addr;
    uint8_t value;
    uint8_t used;
} stage5_validate_mem_entry_t;

#define STAGE5_VALIDATE_MEM_SHADOW_SIZE 4096

typedef struct {
    const dbt_cpu_state_t *cpu;
    stage5_validate_mem_entry_t shadow[STAGE5_VALIDATE_MEM_SHADOW_SIZE];
    uint32_t used_count;
    stage5_validate_mem_status_t status;
} stage5_validate_mem_t;
...
static void stage5_validate_mem_init(stage5_validate_mem_t *m, const dbt_cpu_state_t *cpu) {
    memset(m, 0, sizeof(*m));
    m->cpu = cpu;
    m->status = STAGE5_VALIDATE_MEM_OK;
}

static inline bool stage5_validate_is_mmio_addr(const dbt_cpu_state_t *cpu, uint32_t addr) {
    if (!cpu || !cpu->mmio_enabled) return false;
    uint32_t base = cpu->mmio_base;
    return addr >= base && addr < (base + 0x10000u);
}

static bool stage5_validate_mem_probe_slot(const stage5_validate_mem_t *m,
                                           uint32_t addr,
                                           size_t *slot_out,
                                           bool *found_out) {
    size_t idx = (size_t)(addr * 2654435761u) & (STAGE5_VALIDATE_MEM_SHADOW_SIZE - 1u);
    for (size_t probe = 0; probe < STAGE5_VALIDATE_MEM_SHADOW_SIZE; probe++) {
        size_t slot = (idx + probe) & (STAGE5_VALIDATE_MEM_SHADOW_SIZE - 1u);
        const stage5_validate_mem_entry_t *e = &m->shadow[slot];
        if (!e->used) {
            *slot_out = slot;
            *found_out = false;
            return true;
...
static bool stage5_validate_mem_read_u8(stage5_validate_mem_t *m, uint32_t addr, uint8_t *value) {
    if (!m || !m->cpu || !value) return false;
    if (stage5_validate_is_mmio_addr(m->cpu, addr)) {
        m->status = STAGE5_VALIDATE_MEM_MMIO;
        return false;
    }
    if (addr >= m->cpu->mem_size) {
        m->status = STAGE5_VALIDATE_MEM_OOB;
        return false;
    }

    size_t slot = 0;
    bool found = false;
    if (!stage5_validate_mem_probe_slot(m, addr, &slot, &found)) {
        m->status = STAGE5_VALIDATE_MEM_CAPACITY;
        return false;
    }
    if (found) {
        *value = m->shadow[slot].value;
        return true;
    }
    *value = m->cpu->mem_base[addr];
    return true;
}

static bool stage5_validate_mem_write_u8(stage5_validate_mem_t *m, uint32_t addr, uint8_t value) {
    ...
    if (!found) {
        m->shadow[slot].used = 1;
        m->shadow[slot].addr = addr;
        m->used_count++;
    }
    m->shadow[slot].value = value;
    return true;
}

static bool stage5_validate_mem_load(stage5_validate_mem_t *m, uint32_t addr, uint32_t size, uint32_t *out) {
    if (!m || !out || (size != 1 && size != 2 && size != 4)) return false;
    if (size > m->cpu->mem_size || addr > m->cpu->mem_size - size) {
        m->status = STAGE5_VALIDATE_MEM_OOB;
        return false;
    }
    uint32_t v = 0;
    for (uint32_t i = 0; i < size; i++) {
        uint8_t b = 0;
        if (!stage5_validate_mem_read_u8(m, addr + i, &b)) {
            return false;
        }
        v |= ((uint32_t)b) << (8u * i);
    }
...
static bool stage5_validate_mem_store(stage5_validate_mem_t *m, uint32_t addr, uint32_t size, uint32_t value) {
    if (!m || (size != 1 && size != 2 && size != 4)) return false;
    if (size > m->cpu->mem_size || addr > m->cpu->mem_size - size) {
        m->status = STAGE5_VALIDATE_MEM_OOB;
        return false;
    }
    for (uint32_t i = 0; i < size; i++) {
        uint8_t b = (uint8_t)((value >> (8u * i)) & 0xFFu);
        if (!stage5_validate_mem_write_u8(m, addr + i, b)) {
            return false;
        }
    }
    return true;
...
static bool stage5_validate_mem_has_addr(const stage5_validate_mem_t *m, uint32_t addr, uint8_t *value_out) {
    size_t slot = 0;
    bool found = false;
    if (!stage5_validate_mem_probe_slot(m, addr, &slot, &found) || !found) {
        return false;
    }
    if (value_out) *value_out = m->shadow[slot].value;
    return true;
}

static bool stage5_validate_shadow_equal(const stage5_validate_mem_t *a,
                                         const stage5_validate_mem_t *b) {
    if (!a || !b) return false;
    if (a->used_count != b->used_count) return false;
...
static bool stage5_validate_execute_op(stage5_validate_state_t *s,
                                       stage5_validate_mem_t *m,
                                       uint8_t opcode,
                                       uint8_t rd,
                                       uint8_t rs1_idx,
                                       uint8_t rs2_idx,
    ...
    uint32_t rs1 = stage5_validate_reg_read(s, rs1_idx);
    uint32_t rs2 = stage5_validate_reg_read(s, rs2_idx);

    switch (opcode) {
        case OP_ADD:  stage5_validate_reg_write(s, rd, rs1 + rs2); break;
        case OP_SUB:  stage5_validate_reg_write(s, rd, rs1 - rs2); break;
        case OP_XOR:  stage5_validate_reg_write(s, rd, rs1 ^ rs2); break;
        case OP_OR:   stage5_validate_reg_write(s, rd, rs1 | rs2); break;
        case OP_AND:  stage5_validate_reg_write(s, rd, rs1 & rs2); break;
        case OP_SLL:  stage5_validate_reg_write(s, rd, rs1 << (rs2 & 31)); break;
        case OP_SRL:  stage5_validate_reg_write(s, rd, rs1 >> (rs2 & 31)); break;
        case OP_SRA:  stage5_validate_reg_write(s, rd, (uint32_t)((int32_t)rs1 >> (rs2 & 31))); break;
        case OP_SLT:  stage5_validate_reg_write(s, rd, ((int32_t)rs1 < (int32_t)rs2) ? 1u : 0u); break;
        case OP_SLTU: stage5_validate_reg_write(s, rd, (rs1 < rs2) ? 1u : 0u); break;
        case OP_SEQ:  stage5_validate_reg_write(s, rd, (rs1 == rs2) ? 1u : 0u); break;
        case OP_SNE:  stage5_validate_reg_write(s, rd, (rs1 != rs2) ? 1u : 0u); break;
        case OP_SGT:  stage5_validate_reg_write(s, rd, ((int32_t)rs1 > (int32_t)rs2) ? 1u : 0u); break;
        case OP_SGTU: stage5_validate_reg_write(s, rd, (rs1 > rs2) ? 1u : 0u); break;
        case OP_SLE:  stage5_validate_reg_write(s, rd, ((int32_t)rs1 <= (int32_t)rs2) ? 1u : 0u); break;
        case OP_SLEU: stage5_validate_reg_write(s, rd, (rs1 <= rs2) ? 1u : 0u); break;
        case OP_SGE:  stage5_validate_reg_write(s, rd, ((int32_t)rs1 >= (int32_t)rs2) ? 1u : 0u); break;
        case OP_SGEU: stage5_validate_reg_write(s, rd, (rs1 >= rs2) ? 1u : 0u); break;
        case OP_ADDI: stage5_validate_reg_write(s, rd, rs1 + (uint32_t)imm); break;
        case OP_ORI:  stage5_validate_reg_write(s, rd, rs1 | (uint32_t)imm); break;
        case OP_ANDI: stage5_validate_reg_write(s, rd, rs1 & (uint32_t)imm); break;
        case OP_XORI: stage5_validate_reg_write(s, rd, rs1 ^ (uint32_t)imm); break;
        case OP_SLLI: stage5_validate_reg_write(s, rd, rs1 << (imm & 31)); break;
        case OP_SRLI: stage5_validate_reg_write(s, rd, rs1 >> (imm & 31)); break;
        case OP_SRAI: stage5_validate_reg_write(s, rd, (uint32_t)((int32_t)rs1 >> (imm & 31))); break;
        case OP_SLTI: stage5_validate_reg_write(s, rd, ((int32_t)rs1 < imm) ? 1u : 0u); break;
        case OP_SLTIU:stage5_validate_reg_write(s, rd, (rs1 < (uint32_t)imm) ? 1u : 0u); break;
        case OP_LUI:  stage5_validate_reg_write(s, rd, (uint32_t)imm); break;
        case OP_MUL:  stage5_validate_reg_write(s, rd, rs1 * rs2); break;
        case OP_MULHU: {
            uint64_t p = (uint64_t)rs1 * (uint64_t)rs2;
            stage5_validate_reg_write(s, rd, (uint32_t)(p >> 32));
            break;
        }
        case OP_DIV:
            if (rs2 == 0) {
                stage5_validate_reg_write(s, rd, 0xFFFFFFFFu);
            } else if (rs1 == 0x80000000u && rs2 == 0xFFFFFFFFu) {
                stage5_validate_reg_write(s, rd, 0x80000000u);
            } else {
                stage5_validate_reg_write(s, rd, (uint32_t)((int32_t)rs1 / (int32_t)rs2));
            }
            break;
        case OP_REM:
            if (rs2 == 0) {
                stage5_validate_reg_write(s, rd, rs1);
            } else if (rs1 == 0x80000000u && rs2 == 0xFFFFFFFFu) {
                stage5_validate_reg_write(s, rd, 0);
            } else {
                stage5_validate_reg_write(s, rd, (uint32_t)((int32_t)rs1 % (int32_t)rs2));
            }
            break;
        case OP_LDB: {
            uint32_t v = 0;
            if (!stage5_validate_mem_load(m, rs1 + (uint32_t)imm, 1, &v)) return false;
            stage5_validate_reg_write(s, rd, (uint32_t)(int32_t)(int8_t)(v & 0xFFu));
            break;
        }
        case OP_LDH: {
            uint32_t v = 0;
            if (!stage5_validate_mem_load(m, rs1 + (uint32_t)imm, 2, &v)) return false;
            stage5_validate_reg_write(s, rd, (uint32_t)(int32_t)(int16_t)(v & 0xFFFFu));
            break;
        }
        case OP_LDW: {
            uint32_t v = 0;
            if (!stage5_validate_mem_load(m, rs1 + (uint32_t)imm, 4, &v)) return false;
            stage5_validate_reg_write(s, rd, v);
            break;
        }
        case OP_LDBU: {
            uint32_t v = 0;
            if (!stage5_validate_mem_load(m, rs1 + (uint32_t)imm, 1, &v)) return false;
            stage5_validate_reg_write(s, rd, v & 0xFFu);
            break;
        }
        case OP_LDHU: {
            uint32_t v = 0;
            if (!stage5_validate_mem_load(m, rs1 + (uint32_t)imm, 2, &v)) return false;
            stage5_validate_reg_write(s, rd, v & 0xFFFFu);
            break;
        }
        case OP_STB:
            if (!stage5_validate_mem_store(m, rs1 + (uint32_t)imm, 1, rs2)) return false;
            break;
        case OP_STH:
            if (!stage5_validate_mem_store(m, rs1 + (uint32_t)imm, 2, rs2)) return false;
            break;
        case OP_STW:
...
static bool stage5_validate_execute_decoded(stage5_validate_state_t *s,
                                            ...
                                            uint32_t pc) {
    return stage5_validate_execute_op(s, m, inst->opcode, inst->rd, inst->rs1, inst->rs2,
                                      inst->imm, pc);
}
...
static bool stage5_validate_execute_ir(stage5_validate_state_t *s,
                                       ...
                                       int32_t imm,
                                       uint32_t pc) {
    return stage5_validate_execute_op(s, m, opcode, n->rd, n->rs1, n->rs2, imm, pc);
}
...
static bool stage5_validate_region(const translate_ctx_t *ctx,
    ...
    stage5_validate_mem_init(&ref_mem, ctx->cpu);
    stage5_validate_mem_init(&ir_mem, ctx->cpu);
    ...
    for (uint32_t step = 0; step < region->guest_inst_count; step++) {
        ...
        if (ref.pc != ir.pc ||
            memcmp(ref.regs, ir.regs, sizeof(ref.regs)) != 0 ||
            !stage5_validate_shadow_equal(&ref_mem, &ir_mem)) {
            int first_reg = -1;
            for (int r = 0; r < 32; r++) {
            ...
            if (ref_mem.used_count != ir_mem.used_count) {
...
static inline bool stage5_emit_single_terminal(translate_ctx_t *ctx,
                                                uint8_t opcode,
                                                uint8_t rd,
                                                uint8_t rs1,
                                                uint8_t rs2,
    ...
    switch (opcode) {
        ...
        case OP_HALT:
            ...
            return true;
        case OP_DEBUG:
            translate_debug(ctx, rs1);
            return true;
        case OP_YIELD:
            translate_yield(ctx);
            return true;
        case OP_BEQ:
            return stage5_translate_beq_compact(ctx, rs1, rs2, imm);
        case OP_BNE:
            return stage5_translate_bne_compact(ctx, rs1, rs2, imm);
        case OP_BLT:
        case OP_BGE:
        case OP_BLTU:
        case OP_BGEU:
...
static inline bool stage5_emit_prefix_nonbranch(translate_ctx_t *ctx,
                                                 uint8_t opcode,
                                                 uint8_t rd,
                                                 uint8_t rs1,
                                                 uint8_t rs2,
...
static bool stage5_prefilter_has_cmp_branch_zero(translate_ctx_t *ctx,
                                                 uint32_t guest_pc,
                                                 int max_scan_insts) {
    if (!ctx || max_scan_insts <= 1) return false;
    uint32_t pc = guest_pc;
    uint8_t cmp_rd = 0;
    bool cmp_valid = false;

    for (int i = 0; i < max_scan_insts; i++) {
        if (pc >= ctx->cpu->code_limit || (ctx->cpu->code_limit - pc) < 4) break;
        uint32_t raw = *(uint32_t *)(ctx->cpu->mem_base + pc);
        decoded_inst_t inst = decode_instruction(raw);

        if (stage5_prefilter_is_cmp_opcode(inst.opcode) && inst.rd != 0) {
            cmp_rd = inst.rd;
            cmp_valid = true;
        } else if ((inst.opcode == OP_BEQ || inst.opcode == OP_BNE) && cmp_valid) {
            bool uses_cmp = (inst.rs1 == cmp_rd && inst.rs2 == 0) ||
                            (inst.rs2 == cmp_rd && inst.rs1 == 0);
            if (uses_cmp) {
                return true;
            }
            // Unmatched branch terminates the straight-line candidate.
            break;
        } else {
            if (cmp_valid && inst.rd == cmp_rd && inst.rd != 0) {
                cmp_valid = false;
            }
            // Any control-flow op ends the scan window.
            if (inst.opcode == OP_JAL || inst.opcode == OP_JALR ||
                inst.opcode == OP_BEQ || inst.opcode == OP_BNE ||
                inst.opcode == OP_BLT || inst.opcode == OP_BGE ||
                inst.opcode == OP_BLTU || inst.opcode == OP_BGEU ||
                inst.opcode == OP_HALT || inst.opcode == OP_DEBUG ||
...
static bool stage5_prefilter_has_near_terminal(translate_ctx_t *ctx,
                                               uint32_t guest_pc,
                                               int max_scan_insts) {
    if (!ctx || max_scan_insts <= 0) return false;
    uint32_t pc = guest_pc;
    for (int i = 0; i < max_scan_insts; i++) {
        if (pc >= ctx->cpu->code_limit || (ctx->cpu->code_limit - pc) < 4) break;
        uint32_t raw = *(uint32_t *)(ctx->cpu->mem_base + pc);
...
static bool stage5_try_emit_pilot(translate_ctx_t *ctx, uint32_t guest_pc) {
    ...
    // Cheap prefilter: skip Stage5 work for opcode families currently owned by
    // Stage4 policy.
    uint32_t raw0_pref = *(uint32_t *)(ctx->cpu->mem_base + guest_pc);
    decoded_inst_t inst_pref = decode_instruction(raw0_pref);
    bool is_branch_op =
        inst_pref.opcode == OP_BEQ || inst_pref.opcode == OP_BNE ||
        inst_pref.opcode == OP_BLT || inst_pref.opcode == OP_BGE ||
        inst_pref.opcode == OP_BLTU || inst_pref.opcode == OP_BGEU;
    bool allow_branch_probe =
        ctx->superblock_enabled &&
        is_branch_op &&
        inst_pref.imm > 0;
    bool is_cmp_op = stage5_prefilter_is_cmp_opcode(inst_pref.opcode);
    bool is_small_terminal_start =
        inst_pref.opcode == OP_HALT || inst_pref.opcode == OP_DEBUG ||
        inst_pref.opcode == OP_YIELD ||
    ...
    bool bench_allow_jalr_ret_short =
        bench_profile &&
        emitted_pattern == STAGE5_BURG_PATTERN_JALR_RET_SHORT &&
        stage5_bench_max_jalr_ret_short_ginst() > 0 &&
        region.guest_inst_count <= stage5_bench_max_jalr_ret_short_ginst();
    bool bench_allow_jalr_ret_long =
        bench_profile &&
        emitted_pattern == STAGE5_BURG_PATTERN_JALR_RET_LONG &&
    ...
    if (term_idx > 0 && region.guest_inst_count == 2 &&
        ...
        uint8_t branch_opcode = b->opcode;
        int extra_skip_idx = -1;
        int fuse_cmp_idx = stage5_find_cmp_branch_fused_info(&region, term_idx, result.pattern,
                                                             &branch_opcode, &extra_skip_idx);
        if (fuse_cmp_idx >= 0) {
            c = &region.ir[fuse_cmp_idx];
            cmp_is_imm = (c->opcode == OP_SLTI || c->opcode == OP_SLTIU);
        }
    ...
    if (region.guest_inst_count == 1) {
        ...
        if (stage5_emit_single_terminal(ctx, inst0.opcode, inst0.rd, inst0.rs1, inst0.rs2,
                                        inst0.imm, emitted_pattern)) {
            ctx->superblock_enabled = saved_superblock_enabled;
            stage5_record_emit_success(ctx, emitted_pattern, region.guest_inst_count, emit_start_size);
            if (regflow_retry_applied) stage5_record_regflow_retry_emit_success(regflow_retry_choice);
            return true;
        ...
        if (emitted_pattern == STAGE5_BURG_PATTERN_CMP_BRANCH_ZERO ||
            ...
            int extra_skip_idx = -1;
            int fuse_cmp_idx = stage5_find_cmp_branch_fused_info(&region, terminal_idx, emitted_pattern,
                                                                 &branch_opcode, &extra_skip_idx);
            if (fuse_cmp_idx >= 0 && terminal_idx >= 0) {
                stage5_ir_node_t *c = &region.ir[fuse_cmp_idx];
                stage5_ir_node_t *b = &region.ir[terminal_idx];
                bool cmp_is_imm = (c->opcode == OP_SLTI || c->opcode == OP_SLTIU);
                ctx->guest_pc = b->pc;
                ctx->current_inst_idx = terminal_idx;
                bool ended = stage5_emit_cmp_branch_fused(ctx,
                    c->opcode, c->rs1, c->rs2, c->imm, cmp_is_imm,
                    branch_opcode ? branch_opcode : b->opcode, b->imm, b->pc);
                if (ended) {
                    ctx->superblock_enabled = saved_superblock_enabled;
                    stage5_record_emit_success(ctx, emitted_pattern, region.guest_inst_count, emit_start_size);
                    if (regflow_retry_applied) stage5_record_regflow_retry_emit_success(regflow_retry_choice);
        ...
        if (branch_uses_cmp_rd) {
            ...
            if (branch_uses_cmp_rd) {
                ...
                ctx->current_inst_idx = 1;
                    ...
                    ? stage5_translate_beq_compact(ctx, inst1.rs1, inst1.rs2, inst1.imm)
                    : stage5_translate_bne_compact(ctx, inst1.rs1, inst1.rs2, inst1.imm);
                if (ended) {
                    ctx->superblock_enabled = saved_superblock_enabled;
                    stage5_record_emit_success(ctx, emitted_pattern, region.guest_inst_count, emit_start_size);
                    if (regflow_retry_applied) stage5_record_regflow_retry_emit_success(regflow_retry_choice);
        ...
        if (!branch_uses_cmp_rd) {
            ...
            switch (inst0.opcode) {
                ...
                case OP_BGEU: {
                    ...
                    if ((inst0.opcode == OP_BEQ || inst0.opcode == OP_BNE) &&
                        (inst0.rs1 == 0 || inst0.rs2 == 0)) {
                        uint8_t rz = (inst0.rs1 == 0) ? inst0.rs2 : inst0.rs1;
                        x64_reg_t hz = (rz != 0) ? guest_host_reg(ctx, rz) : X64_NOREG;
                        if (rz == 0) {
                            flush_pending_write(ctx);
                            emit_xor_r32_r32(e, RAX, RAX);
                    ...
                    uint8_t inv_cc = stage5_branch_inv_cc(inst0.opcode);
                    emit_jcc_short(e, inv_cc, 5);
                    size_t jmp_patch = emit_offset(e) + 1;
                    emit_jmp_rel32(e, 0);
        ...
        if ((emitted_pattern == STAGE5_BURG_PATTERN_CMP_BRANCH_ZERO ||
             emitted_pattern == STAGE5_BURG_PATTERN_CMP_BRANCH_XOR1) &&
            terminal_idx > 0) {
            fuse_cmp_idx = stage5_find_cmp_branch_fused_info(&region, terminal_idx, emitted_pattern,
                                                             &fuse_branch_opcode, &fuse_extra_skip_idx);
        }
        ...
        for (uint32_t i = 0; i < region.ir_count; i++) {
            ...
            if (fuse_extra_skip_idx >= 0 && (int)i == fuse_extra_skip_idx) continue;
            ...
            if (side_exit_emit_enabled &&
                ...
                // Emit compare for side-exit branch.
                // For BEQ/BNE with one zero operand, prefer TEST.
        ...
        if (ok_prefix) {
            ...
            if (synth_block_end) {
                ...
                ended = stage5_emit_cmp_branch_fused(ctx,
                    c->opcode, c->rs1, c->rs2, c->imm, cmp_is_imm,
                    (fuse_branch_opcode ? fuse_branch_opcode : b->opcode), b->imm, b->pc);
